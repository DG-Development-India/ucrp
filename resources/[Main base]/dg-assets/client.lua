...
--         SetDiscordAppId(709024022553296947) -- client id (int)
...
--         SetDiscordRichPresenceAsset("round_1024x1024") -- large logo key (string)
--         --SetDiscordRichPresenceAssetText(GetPlayerName(source)) -- Large logo "hover" text (string)

--         SetDiscordRichPresenceAssetSmall("round_512x512") -- small logo key (string)
--         SetDiscordRichPresenceAssetSmallText("Health: "..(GetEntityHealth(player)-100)) -- small logo "hover" text (string)
--         SetDiscordRichPresenceAction(1, "Join Discord", "https://discord.gg/2CB4tfY8ZW")
...
-- 		SetRichPresence(string.format("%s Players", playerCount)) -- main text (string)
-- 		SetDiscordRichPresenceAsset("round_1024x1024") -- large logo key (string)
-- 		SetDiscordRichPresenceAssetText("dg.co.in") -- Large logo "hover" text (string)
-- 		SetDiscordRichPresenceAssetSmall("round_512x512") -- small logo key (string)
-- 		SetDiscordRichPresenceAssetSmallText("dg.co.in") -- small logo "hover" text (string)
--         SetDiscordRichPresenceAction(1, "Join Discord", "https://discord.gg/2CB4tfY8ZW")
-- 	end
-- end)
...
function TeleportToWaypoint()
    DGCore.TriggerServerCallback("tpm:fetchUserRank", function(playerRank)
...
function TaskItem(dictionary,animation,typeAnim,timer,message,func,remove,itemid)
    ...
    exports["dg-taskbar"]:taskBar(timer, message)
...
--                     if DGCore.GetPlayerData().money >= 50 then
...
Citizen.CreateThread(function()
    ...
    while true do
        local ped = PlayerPedId()
        local closestVehicle, Distance = DGCore.Game.GetClosestVehicle()
    ...
    while true do 
        ...
        if Vehicle.Vehicle ~= nil then
                ...
                if IsVehicleSeatFree(Vehicle.Vehicle, -1) and GetVehicleEngineHealth(Vehicle.Vehicle) <= Config.DamageNeeded then
                    DGCore.Game.Utils.DrawText3D({x = Vehicle.Coords.x, y = Vehicle.Coords.y, z = Vehicle.Coords.z}, 'Press [~g~SHIFT~w~] and [~g~E~w~] to push the vehicle', 0.4)
                end
            ...
            if IsControlPressed(0, Keys["LEFTSHIFT"]) and IsVehicleSeatFree(Vehicle.Vehicle, -1) and not IsEntityAttachedToEntity(ped, Vehicle.Vehicle) and IsControlJustPressed(0, Keys["E"])  and GetVehicleEngineHealth(Vehicle.Vehicle) <= Config.DamageNeeded then
                ...
                DGCore.Streaming.RequestAnimDict('missfinale_c2ig_11')
...
Citizen.CreateThread(function()
    while true do
        ...
        if IsPedShooting(ped) then
            if currentWeaponHash ~= GetHashKey("WEAPON_STUNGUN") and currentWeaponHash ~= GetHashKey("WEAPON_BALL") and currentWeaponHash ~= GetHashKey("WEAPON_PETROLCAN") and currentWeaponHash ~= GetHashKey("WEAPON_FIREEXTINGUISHER") then
                if police then
                    TriggerEvent('dg_status:add', 'stress', 5000)
                else
                    TriggerEvent('dg_status:add', 'stress', 20000)
                end
            end
        end
    end
...
--command stuffs
RegisterCommand('gunlicense', function(source, args)
...
---/job
local showingjob = false

RegisterCommand('job', function(source, args)
	if not showingjob then
		showingjob = true
		TriggerServerEvent("dg_base:showjob")
...
RegisterCommand('outfits', function(source, args, rawCommand) 
	local wardrobearea = false
	wardrobearea = getwardrobe()
	if wardrobearea then
		DGCore.TriggerServerCallback('dg_base:getPlayerDressing', function(dressing)
			local elements = {}
			for i=1, #dressing, 1 do
				table.insert(elements, {label = dressing[i], value = i})
			end
			if elements[1] ~= nil then
				DGCore.UI.Menu.Open('default', GetCurrentResourceName(), 'player_dressing', {
					title    = 'Outfits',
					align    = 'top-right',
					elements = elements,
				}, function(data, menu)
					menu.close()
					DGCore.UI.Menu.Open('default', GetCurrentResourceName(), 'outfit_confirm', {
						title    = 'Delete',
						align    = 'top-right',
						elements = { {label = 'Wear',  value = 'wear'}, {label = 'Remove', value = 'remove'} }
						...
						if data2.current.value == 'wear' then
							...
							Citizen.Wait(4000)
							ESX.TriggerServerCallback('dg_base:getPlayerOutfit', function(clothes)
								TriggerEvent('dg_clothes:loadclothes',clothes,false)
							end, data.current.value)
						elseif data2.current.value == 'remove' then
							TriggerServerEvent('dg_base:deleteOutfit', data.current.value,data.current.label)
						end
						menu2.close()
				...
				DGCore.ShowNotification("You have no saved outfits!")
			end
		end)
	end
end)
...
RegisterNetEvent("dg_base:wardrobe")
AddEventHandler("dg_base:wardrobe", function()
	ESX.TriggerServerCallback('dg_base:getPlayerDressing', function(dressing)
		local elements = {}
		...
		if elements[1] ~= nil then
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'player_dressing', {
				...
				menu.close()
				ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'outfit_confirm', {
					title    = 'Delete',
					align    = 'top-right',
					elements = { {label = 'Wear',  value = 'wear'}, {label = 'Remove', value = 'remove'} }
				}, function(data2, menu2)
					...
					elseif data2.current.value == 'remove' then
						TriggerServerEvent('ogrp_base:deleteOutfit', data.current.value,data.current.label)
					end
					menu2.close()
				end, function(data2, menu2)
					menu2.close()
			...
			ESX.ShowNotification("You have no saved outfits!")
		end
	end)
end)

RegisterCommand('clearwardrobe', function(source, args, rawCommand)
	local wardrobearea = false
	wardrobearea = getwardrobe()
	if wardrobearea then
		TriggerServerEvent('ndrp_base:deleteAllOutfits')
	end
end)
