...
RegisterServerEvent('dg-billing:sendBill')
AddEventHandler('dg-billing:sendBill', function(playerId, sharedAccountName, label, amount)
	local _source = source
	local xPlayer = DGCore.GetPlayerFromId(_source)
	local xTarget = DGCore.GetPlayerFromId(playerId)

	TriggerEvent('dg-addonaccount:getSharedAccount', sharedAccountName, function(account)
...
DGCore.RegisterServerCallback('dg-billing:getBills', function(source, cb)
...
DGCore.RegisterServerCallback('dg-billing:getTargetBills', function(source, cb, target)
...
DGCore.RegisterServerCallback('dg-billing:payBill', function(source, cb, id)
	local xPlayer = DGCore.GetPlayerFromId(source)
	...
	MySQL.Async.fetchAll(
		'SELECT * FROM billing WHERE id = @id',
		{
			...
			local xTarget = DGCore.GetPlayerFromIdentifier(sender)
			...
			if targetType == 'player' then
				...
				TriggerEvent('dg-addonaccount:getSharedAccount', target, function(account)
