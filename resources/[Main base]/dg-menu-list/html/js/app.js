(function(){
	...
	window.DGCore_MENU       = {};
	DGCore_MENU.ResourceName = 'dg-menu-list';
	DGCore_MENU.opened       = {};
	DGCore_MENU.focus        = [];
	DGCore_MENU.data         = {};
	...
	DGCore_MENU.render = function() {
		...
		for (let namespace in DGCore_MENU.opened) {
			...
			if (typeof DGCore_MENU.data[namespace] == 'undefined') {
				DGCore_MENU.data[namespace] = {};
			}

			for (let name in DGCore_MENU.opened[namespace]) {

				DGCore_MENU.data[namespace][name] = [];
				...
				for (let i=0; i<menuData.rows.length; i++) {
					...
					DGCore_MENU.data[namespace][name].push(data);
				...
				menu.find('button[data-namespace][data-name]').click(function() {
					DGCore_MENU.submit($(this).data('namespace'), $(this).data('name'), {
						data : DGCore_MENU.data[$(this).data('namespace')][$(this).data('name')][parseInt($(this).data('id'))],
