...
RegisterServerEvent('dg-identity:setIdentity')
AddEventHandler('dg-identity:setIdentity', function(data, myIdentifiers)
	setIdentity(myIdentifiers.steamid, data, function(callback)
		if callback then
			TriggerClientEvent('dg-identity:identityCheck', myIdentifiers.playerid, true)
		else
...
AddEventHandler('es:playerLoaded', function(source)
	...
	TriggerClientEvent('dg-identity:saveID', source, myID)
	getIdentity(source, function(data)
		if data.firstname == '' then
			TriggerClientEvent('dg-identity:identityCheck', source, false)
			TriggerClientEvent('dg-identity:showRegisterIdentity', source)
		else
			TriggerClientEvent('dg-identity:identityCheck', source, true)
		end
	end)
end)
...
AddEventHandler('onResourceStart', function(resource)
	if resource == GetCurrentResourceName() then
		...
		local xPlayers, xPlayer = DGCore.GetPlayers()
		for i=1, #xPlayers, 1 do
			xPlayer = DGCore.GetPlayerFromId(xPlayers[i])
			...
			TriggerClientEvent('dg-identity:saveID', xPlayer.source, myID)

			getIdentity(xPlayer.source, function(data)
				if data.firstname == '' then
					TriggerClientEvent('dg-identity:identityCheck', xPlayer.source, false)
					TriggerClientEvent('dg-identity:showRegisterIdentity', xPlayer.source)
				else
					TriggerClientEvent('dg-identity:identityCheck', xPlayer.source, true)
				end
			end)
		end
	end
