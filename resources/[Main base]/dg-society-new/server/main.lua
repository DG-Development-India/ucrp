...
AddEventHandler('dg_society:withdrawMoney', function(society, amount)
...
AddEventHandler('dg_society:depositMoney', function(society, amount)
	local xPlayer = DGCore.GetPlayerFromId(source)
	local society = GetSociety(society)
...
RegisterServerEvent('dg_society:washMoney')
AddEventHandler('dg_society:washMoney', function(society, amount)
	local xPlayer = DGCore.GetPlayerFromId(source)
...
RegisterServerEvent('dg_society:putVehicleInGarage')
AddEventHandler('dg_society:putVehicleInGarage', function(societyName, vehicle)
	local society = GetSociety(societyName)

	TriggerEvent('dg-datastore:getSharedDataStore', society.datastore, function(store)
...
RegisterServerEvent('dg_society:removeVehicleFromGarage')
AddEventHandler('dg_society:removeVehicleFromGarage', function(societyName, vehicle)
	local society = GetSociety(societyName)
...
DGCore.RegisterServerCallback('dg_society:getEmployees', function(source, cb, society)
	if Config.EnableDGCoreIdentity then
...
DGCore.RegisterServerCallback('dg_society:setJobSalary', function(source, cb, job, grade, salary)
...
DGCore.RegisterServerCallback('dg_society:getVehiclesInGarage', function(source, cb, societyName)
	local society = GetSociety(societyName)
	TriggerEvent('dg-datastore:getSharedDataStore', society.datastore, function(store)
		local garage = store.get('garage') or {}
		cb(garage)
...
DGCore.RegisterServerCallback('dg_society:isBoss', function(source, cb, jobName)
	local xPlayer = DGCore.GetPlayerFromId(source)
...
function WashMoneyCRON(d, h, m)
	MySQL.Async.fetchAll('SELECT * FROM society_moneywash', {}, function(result)
		for i=1, #result, 1 do
			...
			local xPlayer = DGCore.GetPlayerFromIdentifier(result[i].identifier)
			if xPlayer ~= nil then
