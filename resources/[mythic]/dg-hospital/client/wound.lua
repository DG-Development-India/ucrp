...
function CheckDamage(ped, bone, weapon)
    ...
    if Config.Bones[bone] ~= nil then
        if not BodyParts[Config.Bones[bone]].isDamaged then
            ...
            TriggerServerEvent('dg-hospital:server:SyncInjuries', {
                limbs = BodyParts,
...
RegisterNetEvent('dg-hospital:client:SyncBleed')
AddEventHandler('dg-hospital:client:SyncBleed', function(bleedStatus)
    isBleeding = tonumber(bleedStatus)
    DoBleedAlert()
end)

RegisterNetEvent('dg-hospital:client:FieldTreatLimbs')
AddEventHandler('dg-hospital:client:FieldTreatLimbs', function()
    for k, v in pairs(BodyParts) do
        v.isDamaged = false
        v.severity = 1
    end
...
RegisterNetEvent('dg-hospital:client:FieldTreatBleed')
AddEventHandler('dg-hospital:client:FieldTreatBleed', function()
    if isBleeding > 1 then
...
RegisterNetEvent('dg-hospital:client:ReduceBleed')
AddEventHandler('dg-hospital:client:ReduceBleed', function()
    if isBleeding > 0 then
        isBleeding = tonumber(isBleeding) - 1
...
RegisterNetEvent('dg-hospital:client:ResetLimbs')
AddEventHandler('dg-hospital:client:ResetLimbs', function()
    for k, v in pairs(BodyParts) do
        v.isDamaged = false
        v.severity = 0
    end
...
RegisterNetEvent('dg-hospital:client:RemoveBleed')
AddEventHandler('dg-hospital:client:RemoveBleed', function()
    isBleeding = 0
...
RegisterNetEvent('dg-hospital:client:UsePainKiller')
AddEventHandler('dg-hospital:client:UsePainKiller', function(tier)
    if tier < 4 then
...
RegisterNetEvent('dg-hospital:client:UseAdrenaline')
AddEventHandler('dg-hospital:client:UseAdrenaline', function(tier)
    if tier < 4 then
        onDrugs = 180 * tier
    end
