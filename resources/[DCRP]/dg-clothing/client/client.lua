...
AddEventHandler('dg-skin:setLastSkin', function(skin)
    LastSkin = skin
  end)
...
--     local playerhea =  GetEntityHealth(GetPlayerPed(-1))
--     local playerarm = GetPedArmour(GetPlayerPed(-1))
--     LoadYourClothes()
--     Citizen.Wait(3000)
--     SetEntityHealth(PlayerPedId(), playerhea)
--     SetPedArmour(PlayerPedId(), playerarm)
-- end)

-- function LoadYourClothes()
--     DGCore.TriggerServerCallback('dg-clothing:getPlayerSkin', function(skin)
--         if skin ~= nil then
--             LoadPed(skin)
--             Wait(1000)
--             TriggerServerEvent("dg-clothing:retrieve_tats")
-- 	    end
--     end)
-- end
...
function Save(save)
    if save then
        data = GetCurrentPed()
        TriggerServerEvent("dg-clothing:insert_character_current", data)
        if data.model == `mp_f_freemode_01` or data.model == `mp_m_freemode_01` then
            TriggerServerEvent("dg-clothing:insert_character_face", data)
            TriggerServerEvent("dg-clothing:set_tats", currentTats)
        end
    else
        LoadPed(oldPed)
    end
...
RegisterNetEvent("dg-clothing:guardarSkin")
AddEventHandler("dg-clothing:guardarSkin", function()
    data = GetCurrentPed()
    TriggerServerEvent("dg-clothing:insert_character_current", data)
    if data.model == `mp_f_freemode_01` or data.model == `mp_m_freemode_01` then
        TriggerServerEvent("dg-clothing:insert_character_face", data)
...
function IsNearShop(shops)
	...
	for i = 1, #shops do
        ...
        if startingMenu then          
            local comparedst1 = Vdist(plyPos.x, plyPos.y, plyPos.z,-1039.55, -2740.936, 20.16929+0.5)
            if comparedst1 < 3.0 then
                DrawText3D(-1039.55, -2740.936, 20.16929+0.5,'~g~[E]~s~ to change face')
            end
        end

		if comparedst < 5.0 then
			DrawMarker(-1,shop[1], shop[2], shop[3], 0, 0, 0, 0, 0, 0, 1.001, 1.0001, 1.7001, 0, 55, 240, 20, 0, 0, 0, 0)
            if comparedst < 2.0 then
                if shops == clothingShops then
                DrawText3D(shop[1], shop[2], shop[3]+0.5,'~g~[E]~s~ to open Clothes Shop [$150]')
                elseif shops == tattoosShops then
                DrawText3D(shop[1], shop[2], shop[3]+0.5,'~g~[E]~s~ to open Tattoos Shops [$150]')
                elseif shops == barberShops then
                DrawText3D(shop[1], shop[2], shop[3]+0.5,'~g~[E]~s~ to open Barber Shops [$150]')
                -- elseif firstChar == true then
                -- DrawText3D(99.56884, -1712.537, 30.11244+0.5,'~g~[E]~s~ to change face')
                end
            end
		end
	end
...
RegisterNetEvent("dg-clothing:inService")
AddEventHandler("dg-clothing:inService", function(service)
    isService = service
end)

RegisterNetEvent('dg-identity:identityCheck')
AddEventHandler('dg-identity:identityCheck', function(identityCheck)
	hasIdentity = identityCheck
end)

RegisterNetEvent("dg-clothing:hasEnough")
AddEventHandler("dg-clothing:hasEnough", function(menu)
...
RegisterNetEvent("dg-clothing:setclothes")
AddEventHandler("dg-clothing:setclothes", function(data,alreadyExist)
    player = GetPlayerPed(-1)
    local function setDefault()
        Citizen.CreateThread(function()
            firstChar = true
            local gender = nil
            DGCore.TriggerServerCallback('dg-clothing:getSex', function(sex)
				gender = sex
			end)
            Citizen.Wait(1000)
            if gender ~= "m" then
            ...
            DoScreenFadeIn(50)
            Wait(5000)
            if not exports["dg-inventory"]:hasEnoughOfItem("idcard",1,false) then
                TriggerEvent("player:receiveItem","idcard",1,true)
            end
            if not exports["dg-inventory"]:hasEnoughOfItem("mobilephone",1,false)then
    ...
    TriggerServerEvent("dg-clothing:get_character_face")
    TriggerServerEvent("dg-clothing:retrieve_tats")
end)


RegisterNetEvent("dg-clothing:defaultReset")
AddEventHandler("dg-clothing:defaultReset", function()
    local gender = nil
    DGCore.TriggerServerCallback('dg-clothing:getSex', function(sex)
        gender = sex
    end)
...
RegisterNetEvent("dg-clothing:settattoos")
AddEventHandler("dg-clothing:settattoos", function(playerTattoosList)
    currentTats = playerTattoosList
    SetTats(GetTats())
end)

RegisterNetEvent("dg-clothing:setpedfeatures")
AddEventHandler("dg-clothing:setpedfeatures", function(data)
    player = GetPlayerPed(-1)
    if data == false then
...
RegisterCommand("outfituse", function(source, args, rawCommand)
    --if exports["dg-housing"]:imClosesToRoom2() or exports["dg-apartments"]:imClosesToRoom3() or (IsNearShop(clothingShops) < 9.0) then
    if exports["dg_housing"]:imClosesToRoom2() or IsNearShop(clothingShops) < 9.0 then
        if args[1] then	
...
RegisterNetEvent('dg-clothing:listOutfits')
AddEventHandler('dg-clothing:listOutfits', function(skincheck)
...
RegisterNetEvent('dg-clothing:outfits')
AddEventHandler('dg-clothing:outfits', function(pAction, pId, pName)
    if pAction == 1 then
        TriggerServerEvent("dg-clothing:set_outfit",pId, pName, GetCurrentPed())
    elseif pAction == 2 then
        TriggerServerEvent("dg-clothing:remove_outfit",pId)
    elseif pAction == 3 then
        --TriggerEvent('InteractSound_CL:PlayOnOne','Clothes1', 0.6)
        TriggerEvent('InteractSound_CL:PlayOnOne', 'clothe', 0.6)
		TriggerEvent('dg-emotes:playthisemote', 'adjust')
        Citizen.Wait(4000)
        TriggerServerEvent("dg-clothing:get_outfit", pId)
    else
        TriggerServerEvent("dg-clothing:list_outfits")
    end
end)
