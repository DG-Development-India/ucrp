...
TriggerEvent("dg:getSharedObject", function(library) 
	DGCore = library 
end)

RegisterServerEvent('dg-skin:save')
AddEventHandler('dg-skin:save', function()
    local src = source
    TriggerClientEvent('dg-clothing:guardarSkin',src)
end)

DGCore.RegisterServerCallback('dg-skin:getPlayerSkin', function(source, cb)
...
DGCore.RegisterServerCallback('dg-clothing:getSex', function(source, cb)
    local xPlayer = DGCore.GetPlayerFromId(source)
...
RegisterServerEvent("dg-clothing:insert_character_current")
AddEventHandler("dg-clothing:insert_character_current",function(data)
    if not data then return end
    local src = source
    local user = DGCore.GetPlayerFromId(src)
...
RegisterServerEvent("dg-clothing:insert_character_face")
AddEventHandler("dg-clothing:insert_character_face",function(data)
    if not data then return end
    local src = source
...
RegisterServerEvent("dg-clothing:get_character_face")
AddEventHandler("dg-clothing:get_character_face",function(pSrc)
    ...
    MySQL.Async.fetchAll("SELECT cc.model, cf.hairColor, cf.headBlend, cf.headOverlay, cf.headStructure FROM users cf INNER JOIN users cc on cc.identifier = cf.identifier WHERE cf.identifier = @identifier", {['identifier'] = characterId}, function(result)
        if (result ~= nil and result[1] ~= nil) then
            ...
            if model == 1885233650 or model == -1667301416 then
                TriggerClientEvent("dg-clothing:setpedfeatures", src, temp_data)
            end
        else
            TriggerClientEvent("dg-clothing:setpedfeatures", src, false)
        end
	end)
end)

RegisterServerEvent("dg-clothing:get_character_current")
AddEventHandler("dg-clothing:get_character_current",function(pSrc)
    local src = (not pSrc and source or pSrc)
    local user = DGCore.GetPlayerFromId(src)
    if not user then return end
    local characterId = user.identifier
    ...
    MySQL.Async.fetchAll("SELECT * FROM users WHERE identifier = @identifier", {['identifier'] = characterId}, function(result)
        ...
        TriggerClientEvent("dg-clothing:setclothes", src, temp_data,0)
	end)
end)

RegisterServerEvent("dg-clothing:retrieve_tats")
AddEventHandler("dg-clothing:retrieve_tats", function(pSrc)
    local src = (not pSrc and source or pSrc)
    local user = DGCore.GetPlayerFromId(src)
    if not user then return end
	MySQL.Async.fetchAll("SELECT * FROM playerstattoos WHERE identifier = @identifier", {['identifier'] = user.identifier}, function(result)
        if(#result == 1) then
			TriggerClientEvent("dg-clothing:settattoos", src, json.decode(result[1].tattoos))
		else
			local tattooValue = "{}"
			...
			TriggerClientEvent("dg-clothing:settattoos", src, {})
		end
	end)
end)

RegisterServerEvent("dg-clothing:set_tats")
AddEventHandler("dg-clothing:set_tats", function(tattoosList)
	local src = source
	local user = DGCore.GetPlayerFromId(src)
...
RegisterServerEvent("dg-clothing:get_outfit")
AddEventHandler("dg-clothing:get_outfit",function(slot)
    ...
    MySQL.Async.fetchAll("SELECT * FROM character_outfits WHERE steamid = @steamid and slot = @slot", {
        ...
        if result and result[1] then
            ...
            TriggerClientEvent("dg-clothing:setclothes", src, data,0)
...
RegisterServerEvent("dg-clothing:set_outfit")
AddEventHandler("dg-clothing:set_outfit",function(slot, name, data)
    if not slot then return end
    local src = source
...
RegisterServerEvent("dg-clothing:remove_outfit")
AddEventHandler("dg-clothing:remove_outfit",function(slot)
...
RegisterServerEvent("dg-clothing:list_outfits")
AddEventHandler("dg-clothing:list_outfits",function()
    local src = source
    local user = DGCore.GetPlayerFromId(src)
    local steamid = user.identifier
    local slot = slot
    ...
    MySQL.Async.fetchAll("SELECT slot, name FROM character_outfits WHERE steamid = @steamid", {['steamid'] = steamid}, function(skincheck)
    	TriggerClientEvent("dg-clothing:listOutfits",src, skincheck)
	end)
end)
        ...
        if result[1] == nil then
            MySQL.Async.fetchAll("SELECT * FROM users WHERE identifier = @identifier", {["identifier"] = steamid}, function(result)
                if result[1].skin then
                    TriggerClientEvent('dg-clothing:setclothes',src,{},json.decode(result[1].skin))
                else
                    TriggerClientEvent('dg-clothing:setclothes',src,{},nil)
                end
                return
            end)
        else
            TriggerEvent("dg-clothing:get_character_current", src)
        end
        TriggerClientEvent("dg-clothing:inService",src,isService)
    end)
end)
...
AddEventHandler("clothing:checkMoney", function(menu,askingPrice)
    local src = source
    local target = DGCore.GetPlayerFromId(src)
    ...
    if (tonumber(target.getMoney()) >= askingPrice) then
        ...
        TriggerClientEvent("dg-clothing:hasEnough",src,menu)
    else
...
-- 	if xPlayer.getMoney() > (askingPrice - 1) then
-- 		TriggerClientEvent('dg-clothing:hasEnough',src, menu)
-- 	else
