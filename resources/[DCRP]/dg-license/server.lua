...
AddEventHandler('jsfour-idcard:open', function(ID, targetID, type)
	local identifier = DGCore.GetPlayerFromId(ID).identifier
	local _source 	 = DGCore.GetPlayerFromId(targetID).source
	local show       = false
		...
		if (user[1] ~= nil) then
			...
			function (licenses)
				...
				if show then
					...
					TriggerClientEvent('dg:showNotification', _source, "You don't have that type of license..")
				end
			end)
		end
	end)
