...
Citizen.CreateThread(function()
    while DGCore == nil do
        ...
        TriggerEvent("dg:getSharedObject", function(response)
            DGCore = response
        end)
    end

    if DGCore.IsPlayerLoaded() then
...
AddEventHandler("dg:playerLoaded", function(response)
	PlayerData = response
...
RegisterNetEvent("dg-qalle-sellvehicles:refreshVehicles")
AddEventHandler("dg-qalle-sellvehicles:refreshVehicles", function()
	RemoveVehicles()
		...
		while true do
			...
			if dstCheck <= 10.0 then
				...
				if dstCheck <= 4.2 then
					DGCore.Game.Utils.DrawText3D(SellPos, "[E] Open Menu", 0.4)
					if IsControlJustPressed(0, 38) then
						if IsPedInAnyVehicle(ped, false) then
							OpenSellMenu(GetVehiclePedIsUsing(ped))
						else
							DGCore.ShowNotification("You must sit in a ~g~vehicle")
						end
					end
				end
			end
			...
			for i = 1, #Config.VehiclePositions, 1 do
				if Config.VehiclePositions[i]["entityId"] ~= nil then
					...
					if dstCheck <= 2.0 then
						...
						DGCore.Game.Utils.DrawText3D(vehCoords, "[E] " .. Config.VehiclePositions[i]["price"] .. ":-", 0.4)
						...
						if IsControlJustPressed(0, 38) then
							if IsPedInVehicle(ped, Config.VehiclePositions[i]["entityId"], false) then
								...
								DGCore.ShowNotification("You must sit in the ~g~vehicle~s~!")
							end
						end
					end
				end
...
function OpenSellMenu(veh, price, buyVehicle, owner)
	...
	DGCore.UI.Menu.Open('default', GetCurrentResourceName(), 'sell_veh',
		{
			title    = "Vehicle Menu",
			align    = 'top-right',
			elements = elements
	...
	function(data, menu)
		...
		if action == "price" then
			DGCore.UI.Menu.Open('dialog', GetCurrentResourceName(), 'sell_veh_price',
				{
					title = "Vehicle Price"
				},
		...
		elseif action == "sell" then
			local vehProps = DGCore.Game.GetVehicleProperties(veh)

			DGCore.TriggerServerCallback("dg-qalle-sellvehicles:isVehicleValid", function(valid)

				if valid then
					DeleteVehicle(veh)
					DGCore.ShowNotification("You put out the vehicle for sale - " .. price .. " :-")
					menu.close()
				else
					DGCore.ShowNotification("You must own the vehicle! or its Finance Isnt Cleared ")
				end
	
			end, vehProps, price)
		elseif action == "buy" then
			DGCore.TriggerServerCallback("dg-qalle-sellvehicles:buyVehicle", function(isPurchasable, totalMoney)
				if isPurchasable then
					DeleteVehicle(veh)
					DGCore.ShowNotification("You bought the vehicle for " .. price .. " :-")
					menu.close()
				else
					DGCore.ShowNotification("You don't have enough cash, it's missing " .. price - totalMoney .. " :-")
				end
			end, DGCore.Game.GetVehicleProperties(veh), price)
		elseif action == "remove" then
			DGCore.TriggerServerCallback("dg-qalle-sellvehicles:buyVehicle", function(isPurchasable, totalMoney)
				if isPurchasable then
					DeleteVehicle(veh)
					DGCore.ShowNotification("You removed the vehicle")
					menu.close()
				end
			end, DGCore.Game.GetVehicleProperties(veh), 0)
		end
...
function RemoveVehicles()
	...
	for i = 1, #VehPos, 1 do
		local veh, distance = DGCore.Game.GetClosestVehicle(VehPos[i])
...
function SpawnVehicles()
	...
	DGCore.TriggerServerCallback("dg-qalle-sellvehicles:retrieveVehicles", function(vehicles)
		for i = 1, #vehicles, 1 do
			...
			DGCore.Game.SetVehicleProperties(VehPos[i]["entityId"], vehicleProps)
