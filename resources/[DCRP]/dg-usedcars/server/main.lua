...
DGCore.RegisterServerCallback("dg-qalle-sellvehicles:retrieveVehicles", function(source, cb)
	local src = source
	local xPlayer = DGCore.GetPlayerFromId(src)["identifier"]
...
DGCore.RegisterServerCallback("dg-qalle-sellvehicles:isVehicleValid", function(source, cb, vehicleProps, price)
	local src = source
	...
	RetrievePlayerVehicles(xPlayer.identifier, function(ownedVehicles)
		...
		for id, v in pairs(ownedVehicles) do
			...
			if Trim(plate) == Trim(v.plate) and v.finance == 0 and #Config.VehiclePositions ~= VehiclesForSale then
                ...
                TriggerClientEvent("dg-qalle-sellvehicles:refreshVehicles", -1)
...
DGCore.RegisterServerCallback("dg-qalle-sellvehicles:buyVehicle", function(source, cb, vehProps, price)
	local src = source
	local xPlayer = DGCore.GetPlayerFromId(src)
...
function UpdateCash(identifier, cash)
	local xPlayer = DGCore.GetPlayerFromIdentifier(identifier)

	if xPlayer ~= nil then
		xPlayer.addAccountMoney("bank", cash)

		TriggerClientEvent("dg:showNotification", xPlayer.source, "Someone bought your vehicle and transferred $" .. cash)
	else
