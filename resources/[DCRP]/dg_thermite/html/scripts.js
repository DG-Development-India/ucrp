$(document).ready(function(){
  ...
  function playGame(DropAmount,letterSet,speed,intervalSet) {
    ...
    var animations = {'q':[],'w':[],'e':[],'r':[],'j':[],'k':[],'l':[],'a':[],'s':[]};
    ...
    function gameOver() {
      ...
      $.post('http://dg_thermite/failure', JSON.stringify({}));
      gameOn = false;
      clearInterval(cleanupInterval);
      getAllAnimations().forEach(function(anim) {
        anim.pause();
    ...
    function setupNextLetter() {
      ...
      if(currentAmount == 0 && !soundSet)
      {
        ...
        $.post('http://dg_thermite/complete', JSON.stringify({}));
      }
    }
    setupNextLetter();
  }
  ...
  document.onkeyup = function (data) {
    if (data.which == 27 ) {
      $.post('http://dg_thermite/close', JSON.stringify({}));
    }
  };
