...
RegisterServerEvent('dg_jewelrobbery:closestore')
AddEventHandler('dg_jewelrobbery:closestore', function()
    ...
    if ispolice and resettime ~= nil then
        TriggerClientEvent('dg_jewelrobbery:policeclosure', -1)
        policeclosed = true
...
RegisterServerEvent('dg_jewelrobbery:playsound')
AddEventHandler('dg_jewelrobbery:playsound', function(x,y,z, soundtype)
    TriggerClientEvent('dg_jewelrobbery:playsound', -1, x, y, z, soundtype)
end)

RegisterServerEvent('dg_jewelrobbery:setcase')
AddEventHandler('dg_jewelrobbery:setcase', function(casenumber, switch)
    ...
    if not Config.CaseLocations[casenumber].Broken then
        ...
        TriggerEvent('dg_jewelrobbery:RestTimer')
        TriggerClientEvent('dg_jewelrobbery:setcase', -1, casenumber, true)
        TriggerEvent('dg_jewelrobbery:AwardItems', _source)
    end
end)

RegisterServerEvent('dg_jewelrobbery:policenotify')
AddEventHandler('dg_jewelrobbery:policenotify', function()
    TriggerClientEvent('dg_dispatch:jewelrobbery', -1)
	return
end)


RegisterServerEvent('dg_jewelrobbery:loadconfig')
AddEventHandler('dg_jewelrobbery:loadconfig', function()
    local _source = source
    local xPlayer = DGCore.GetPlayerFromId(_source)
    local buildlist = {
        id = _source,
    ...
    TriggerClientEvent('dg_jewelrobbery:loadconfig', _source, Config.CaseLocations)
    if policeclosed then
        TriggerClientEvent('dg_jewelrobbery:policeclosure', _source)
    end

end)

AddEventHandler('dg_jewelrobbery:AwardItems', function(source)
    local _source = source
	...
	TriggerClientEvent("player:receiveItem", _source, "diamond",math.random(1,2))
...
AddEventHandler('dg_jewelrobbery:RestTimer', function()
    if resettime == nil then
        ...
        TriggerClientEvent('dg_jewelrobbery:resetcases', -1, Config.CaseLocations)
        resettime = nil
        policeclosed = false
    end
end)
...
AddEventHandler('dg_jewelrobbery:AwardItems', function(serverid)
    local xPlayer = DGCore.GetPlayerFromId(serverid)
