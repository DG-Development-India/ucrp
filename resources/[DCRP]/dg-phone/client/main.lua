...
RegisterNUICallback('btnTaskGroups', function()
    local jobgrade = DGCore.PlayerData.job.grade
    local rankConverted = DGCore.PlayerData.job.grade_label

    local groupObject = {}
    table.insert(groupObject, {
        namesent = DGCore.PlayerData.job.label,
...
function findVehFromPlateAndLocate(plate)
  ...
  local gameVehicles = DGCore.Game.GetVehicles()
...
RegisterNUICallback('btnAttempCreateHouse', function(data, cb)
  TriggerEvent("dg-housing:client:createHouses", data.price, data.tier)
  loadedHouse = false
...
function hasPhone()
  ...
  if
    (
    (exports["dg-inventory"]:hasEnoughOfItem("mobilephone",1,false) or 
    exports["dg-inventory"]:hasEnoughOfItem("stoleniphone",1,false) or 
    exports["dg-inventory"]:hasEnoughOfItem("stolens8",1,false) or
    exports["dg-inventory"]:hasEnoughOfItem("stolennokia",1,false) or
    exports["dg-inventory"]:hasEnoughOfItem("stolenpixel3",1,false) or
    exports["dg-inventory"]:hasEnoughOfItem("boomerphone",1,false))
...
Citizen.CreateThread(function()
  while true do
    Citizen.Wait(0)
    if IsDisabledControlJustReleased(1, 244) then
      TriggerEvent("dg:heHasPhone")
    end
  end
end)

RegisterNetEvent('dg:heHasPhone')
AddEventHandler('dg:heHasPhone', function()
    GotPhone()
end)
...
function openGuiNow()
  ...
  if hasPhone() then
    ...
    if DGCore.PlayerData.job.name == "realestateagent" then
     isREAgent = true
    end
...
RegisterNUICallback('newPostSubmit', function(data, cb)
  if DGCore.PlayerData.job.name ~= "taxi" and DGCore.PlayerData.job.name ~= "towtruck" and DGCore.PlayerData.job.name ~= "trucker" then
    TriggerServerEvent('phone:updatePhoneJob', data.advert)
  else
    TriggerServerEvent('phone:updatePhoneJob', DGCore.PlayerData.job.name .. " | " .. data.advert)
...
AddEventHandler('phone:addToCall', function(voipchannel)
  ...
  exports.dg_voip:addPlayerToRadio(tonumber(voipchannel))
...
AddEventHandler('phone:removefromToko', function(playerRadioChannel)
  exports.dg_voip:removePlayerFromRadio(playerRadioChannel)
...
AddEventHandler('getAccountInfo', function(cash, bank, licences)
    local responseObject = {
        cash = cash,
        bank = bank,
        job = DGCore.PlayerData.job.name .. ', ' .. DGCore.PlayerData.job.grade_label,
...
RegisterNetEvent("dg-phone:userTwitter")

AddEventHandler("dg-phone:userTwitter", function(user)
  userTwitter = user
 end)
