...
-- Load Spot Status:
RegisterNetEvent('dg_MinerJob:loadSpot')
AddEventHandler('dg_MinerJob:loadSpot', function(list, list2, list3)
    MiningPos = list
    WashingPos = list2
    SmeltingPos = list3
end)
...
RegisterNetEvent('dg_MinerJob:updateStatus')
AddEventHandler('dg_MinerJob:updateStatus', function(id,status)
			...
			if not v.mining and not currentlyMining then
				...
				if GetDistanceBetweenCoords(coords.x, coords.y, coords.z, v.spot[1], v.spot[2], v.spot[3], true) <= 1.0 then
					if IsControlJustPressed(0,Config.KeyToStartMining) and not keyPressed then
						keyPressed = true
						--DGCore.TriggerServerCallback("dg_MinerJob:getPickaxe", function(pickaxe)
						if exports['dg-inventory']:hasEnoughOfItem('pickaxe', 1, true) then
							local stone = exports['dg-inventory']:getQuantity('stones')
							if stone <= 95 then		
								DGCore.TriggerServerCallback("dg_MinerJob:didmining", function(didminingok) 
									if didminingok then
										TriggerServerEvent("dg_MinerJob:spotStatus", k, true)
								...
								TriggerServerEvent("dg_MinerJob:getStoneLimit")
...
function MiningEvent(k,v)
	...
	exports['dg-taskbar']:taskBar(1000, "Mining")
	Citizen.Wait(200)
	...
	TaskPlayAnim(PlayerPedId(), anim, action, 3.0, -3.0, -1, 31, 0, false, false, false)
	Citizen.Wait(2000)
	TriggerEvent( "player:receiveItem", "stones", 5 )
	--TriggerServerEvent("dg_MinerJob:reward",Config.Stone,Config.StoneReward)
	TriggerServerEvent("dg_MinerJob:spotStatus", k, false)
			...
			if not currentlyWashing then
				...
				if GetDistanceBetweenCoords(coords.x, coords.y, coords.z, v.spot[1], v.spot[2], v.spot[3], true) <= 1.2 then
					if IsControlJustPressed(0,Config.KeyToStartWashing) and not keyPressed then
						keyPressed = true
						--DGCore.TriggerServerCallback("dg_MinerJob:getWashPan", function(washPan)
							--if washPan then
							local WStone = exports['dg-inventory']:getQuantity('washedstones')
								--DGCore.TriggerServerCallback("dg_MinerJob:getWashedStoneLimit", function(washedStoneLimitOK)
									if WStone <= 95  then
...
function WashingEvent(k,v)
	...
	--DGCore.TriggerServerCallback("dg_MinerJob:removeStone", function(stoneCount)
	local WStoneCount = exports['dg-inventory']:hasEnoughOfItem('stones', 10, true)
		if WStoneCount then
			-- exports['pogressBar']:drawBar(10000, 'Plaunami akmenys')
			TaskStartScenarioInPlace(playerPed, "PROP_HUMAN_BUM_BIN", 0, true)
			exports['dg-taskbar']:taskBar(1000, "Washing Stones")
			Citizen.Wait(200)
			
			TriggerEvent("inventory:removeItem", "stones", 10)
			Citizen.Wait(10000)
			--Citizen.Wait(1000)
			TriggerEvent("player:receiveItem", "washedstones", 10)
			--TriggerEvent("player:recieveItem", 'washedstone', Config.WStoneReward)
			--TriggerEvent( "player:receiveItem", "heartstopper", 10 )

			--TriggerServerEvent("dg_MinerJob:reward",Config.WStone,Config.WStoneReward)
		else
				...
				if GetDistanceBetweenCoords(coords.x, coords.y, coords.z, v.spot[1], v.spot[2], v.spot[3], true) <= 1.0 then
					if IsControlJustPressed(0,Config.KeyToStartSmelting) and not keyPressed then
						...
						if closestPlayer == -1 or closestDistance >= 0.7 then
							local WStoneCount1 = exports['dg-inventory']:hasEnoughOfItem('washedstones', 10, true)
							if WStoneCount1 then
								TaskStartScenarioInPlace(playerPed, "PROP_HUMAN_BUM_BIN", 0, true)
								TriggerEvent("inventory:removeItem", 'washedstones', 10)
...
function SmeltingEvent()
	...
	exports['dg-taskbar']:taskBar(10000, "Smelting Stones")
	TaskStartScenarioInPlace(playerPed, "PROP_HUMAN_BUM_BIN", 0, true)
	--Citizen.Wait(10000)
	local rewardChance = math.random(1,200)
	if rewardChance > 150 and rewardChance <= 170 then
		TriggerEvent("player:receiveItem", 'diamondore', 1)
	elseif rewardChance >150 and rewardChance <= 160 then
		TriggerEvent("player:receiveItem", 'goldore', 2)
	elseif rewardChance > 120 and rewardChance <= 150 then
		TriggerEvent("player:receiveItem", 'silverore', 2)
	elseif rewardChance > 80 and rewardChance <= 120 then
		TriggerEvent("player:receiveItem", 'ironore', 5)
	elseif rewardChance > 40 and rewardChance <= 80 then
		TriggerEvent("player:receiveItem", 'aluminiumore', 10)
	elseif rewardChance <= 40 then
		TriggerEvent("player:receiveItem", 'copperore', 10)
	end
	--TriggerServerEvent("dg_MinerJob:rewardSmelting")
...
-- Refresh Spots:
Citizen.CreateThread(function()
    TriggerServerEvent("dg_MinerJob:refreshSpots")
end)
