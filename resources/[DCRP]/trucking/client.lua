...
Citizen.CreateThread(function()
    while DGCore == nil do TriggerEvent('dg:getSharedObject', function(obj) DGCore = obj end) Wait(0) end
    while DGCore.GetPlayerData().job == nil do Wait(0) end
    ...
    while true do
        ...
        if distance <= 25.0 and hasjob then
            sleep = 0
            --DrawMarker(20, Config.JobCenter, vector3(0.0, 0.0, 0.0), vector3(0.0, 0.0, 0.0), vector3(1.0, 1.0, 1.0), 255, 0, 255, 150, false, false, 2, false, false, false)
            if distance <= 1.5 then 
                helpText(Strings['e_browse_jobs'])
                if IsControlJustReleased(0, 38) then
                    TriggerServerEvent('dg_ets:jobMenu')
                end
            end
        end
        Wait(sleep)
...
RegisterNetEvent('dg_ets:start')
AddEventHandler('dg_ets:start', function(data, jobid)
    working = true
...
--            text(Strings['get_to_truck'], 250)
    ...
    DeleteVehicle(truck)
    working = false
    TriggerServerEvent('dg_ets:jobFinished', jobid, damages)
end)

RegisterNetEvent('dg_ets:menu')
AddEventHandler('dg_ets:menu', function(data)
    ...
    DGCore.UI.Menu.Open(
        ...
        function(data, menu)
            TriggerServerEvent('dg_ets:startJob', data.current.value)
            menu.close()
        end,
    function(data, menu)
        menu.close()
