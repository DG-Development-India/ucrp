...
AddEventHandler('playerSpawned', function()
	...
	if FirstSpawn then
		...
		DGCore.TriggerServerCallback('dg-ambulancejob:getDeathStatus', function(isDead)
...
function OnPlayerDeath()
	...
	TriggerServerEvent('dg-ambulancejob:setDeathState', true)
...
function resetrelations()
    Citizen.Wait(1000)
    if DGCore.GetPlayerData().job.name == 'police' or DGCore.GetPlayerData().job.name == 'ambulance' then
...
AddEventHandler('disableAllActions2', function()
        ...
        while IsDead do
			...
			DisableControlAction(0, Keys['E'], true) -- E
			DisableControlAction(0, Keys['Z'], true) -- Z
...
function releaseBody()
    ...
    DGCore.TriggerServerCallback("dg-jail:retrieveTime", function(inJail, newJailTime)
		if inJail then
...
function StartDeathTimer()
	...
	if Config.EarlyRespawnFine then
		DGCore.TriggerServerCallback('dg-ambulancejob:checkBalance', function(canPay)
			canPayFine = canPay
		end)
	end

	local earlySpawnTimer = DGCore.Math.Round(Config.EarlyRespawnTimer / 1000)
	local bleedoutTimer = DGCore.Math.Round(Config.BleedoutTimer / 1000)
	...
	Citizen.CreateThread(function()
		...
		while bleedoutTimer > 0 and IsDead do
			...
			elseif Config.EarlyRespawnFine and canPayFine then
				text = text .. _U('respawn_bleedout_fine', DGCore.Math.GroupDigits(Config.EarlyRespawnFineAmount))

				if IsControlPressed(0, Keys['H']) and timeHeld > 60 then
					TriggerServerEvent('dg-ambulancejob:payFine')
					RemoveItemsAfterRPDeath()
...
function RemoveItemsAfterRPDeath()
	TriggerServerEvent('dg-ambulancejob:setDeathState', false)
	...
	Citizen.CreateThread(function()
		...
		DGCore.TriggerServerCallback('dg-ambulancejob:removeItemsAfterRPDeath', function()
			local playerpos = GetEntityCoords( GetPlayerPed(-1) )
				
			DGCore.SetPlayerData('lastPosition', playerpos)
			DGCore.SetPlayerData('loadout', {})
			RespawnPed(PlayerPedId(), playerpos, Config.RespawnPoint.heading)

			TriggerServerEvent('dg:updateLastPosition', playerpos)
...
RegisterNetEvent('dg-ambulancejob:useItem')
AddEventHandler('dg-ambulancejob:useItem', function(itemName)
	DGCore.UI.Menu.CloseAll()
	...
	if itemName == 'medikit' then
		...
		exports["dg-taskbar"]:taskBar(10000, "Healing")
	
			TriggerEvent('dg-ambulancejob:heal', 'big', true)
			TriggerEvent('dg-hospital:client:RemoveBleed')
			TriggerEvent('dg-hospital:client:ResetLimbs')
	...
	elseif itemName == 'bandage' then
		...
		exports["dg-taskbar"]:taskBar(10000, "Healing")
			...
			TriggerEvent('dg-ambulancejob:heal', 'small', true)
			TriggerEvent('dg-hospital:client:RemoveBleed')
			ClearPedTasks(playerPed)
	end
end)
...
function RespawnPed(ped, coords, heading)
	...
	TriggerEvent('dg-hospital:client:RemoveBleed', ped) 
	TriggerEvent('dg-hospital:client:ResetLimbs', ped)

	DGCore.UI.Menu.CloseAll()
end

AddEventHandler('dg:onPlayerDeath', function(data)
	OnPlayerDeath()
end)
...
RegisterNetEvent('dg-ambulancejob:revive')
AddEventHandler('dg-ambulancejob:revive', function()
	attemptRevive()
end)
