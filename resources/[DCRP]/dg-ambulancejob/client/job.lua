...
RegisterNetEvent('dg-ambulancejob:drag')
AddEventHandler('dg-ambulancejob:drag', function(copID)
...
RegisterCommand('escort', function()
	...
	if closestPlayer ~= -1 and closestDistance <= 3.0 then
		TriggerServerEvent('dg-ambulancejob:drag', GetPlayerServerId(closestPlayer))
	else
...
function OpenAmbulanceActionsMenu()
	...
	if Config.EnablePlayerManagement and DGCore.PlayerData.job.grade_name == 'boss' then
		table.insert(elements, {label = _U('boss_actions'), value = 'boss_actions'})
	end

	DGCore.UI.Menu.CloseAll()

	DGCore.UI.Menu.Open('default', GetCurrentResourceName(), 'ambulance_actions', {
		title    = _U('ambulance'),
		align    = 'bottom-right',
		elements = elements
	}, function(data, menu)
		...
		elseif data.current.value == 'boss_actions' then
			TriggerEvent('dg-society:openBossMenu', 'ambulance', function(data, menu)
				menu.close()
			end, {wash = false})
		end
...
function OpenMobileAmbulanceActionsMenu()
	...
	DGCore.UI.Menu.Open('default', GetCurrentResourceName(), 'mobile_ambulance_actions', {
		title    = _U('ambulance'),
		align    = 'bottom-right',
		elements = {
		...
		if data.current.value == 'citizen_interaction' then
			DGCore.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction', {
				...
				if closestPlayer == -1 or closestDistance > 2.0 then
					...
					if data.current.value == 'revive' then
						...
						DGCore.TriggerServerCallback('dg-ambulancejob:getItemAmount', function(quantity)
							if quantity > 0 then
								...
								if IsEntityPlayingAnim(closestPlayerPed, "dead", "dead_a", 3) or IsEntityPlayingAnim(closestPlayerPed, "dead", "dead_b", 3) or IsEntityPlayingAnim(closestPlayerPed, "dead", "dead_c", 3) or IsEntityPlayingAnim(closestPlayerPed, "dead", "dead_d", 3) or IsEntityPlayingAnim(closestPlayerPed, "dead", "dead_e", 3) or IsEntityPlayingAnim(closestPlayerPed, "dead", "dead_f", 3) or IsEntityPlayingAnim(closestPlayerPed, "dead", "dead_g", 3) or IsEntityPlayingAnim(closestPlayerPed, "dead", "dead_h", 3)then
									...
									TriggerServerEvent('dg-ambulancejob:removeItem', 'medikit')
									TriggerServerEvent('dg-ambulancejob:revive', GetPlayerServerId(closestPlayer))
						...
						DGCore.TriggerServerCallback('dg-ambulancejob:getItemAmount', function(quantity)
							if quantity > 0 then
								...
								if health > 0 then
									...
									TriggerServerEvent('dg-ambulancejob:removeItem', 'bandage')
									TriggerServerEvent('dg-ambulancejob:heal', GetPlayerServerId(closestPlayer), 'small')
						...
						DGCore.TriggerServerCallback('dg-ambulancejob:getItemAmount', function(quantity)
							if quantity > 0 then
								...
								if health > 0 then
									...
									TriggerServerEvent('dg-ambulancejob:removeItem', 'medikit')
									TriggerServerEvent('dg-ambulancejob:heal', GetPlayerServerId(closestPlayer), 'big')
					...
					elseif data.current.value == 'put_in_vehicle' then
						TriggerServerEvent('dg-ambulancejob:putInVehicle', GetPlayerServerId(closestPlayer))
					elseif data.current.value == 'pull_out_vehicle' then
						TriggerServerEvent('dg-ambulancejob:pullOutVehicle', GetPlayerServerId(closestPlayer))
					elseif data.current.value == 'drag_player' then
						TriggerServerEvent('dg-ambulancejob:drag', GetPlayerServerId(closestPlayer))
					elseif data.current.value == 'un_drag' then
						TriggerServerEvent('dg-ambulancejob:undrag', GetPlayerServerId(closestPlayer))
					end
				end
			end, function(data, menu)
				menu.close()
...
function FastTravel(coords, heading)
	...
	DGCore.Game.Teleport(playerPed, coords, function()
		DoScreenFadeIn(800)
	...
	while true do
		...
		if DGCore.PlayerData.job and DGCore.PlayerData.job.name == 'ambulance' then
			...
			for hospitalNum,hospital in pairs(Config.Hospitals) do
				...
				for k,v in ipairs(hospital.AmbulanceActions) do
					...
					if distance < Config.Marker.x then
						...
						--DrawText3Ds(coord.x, coord.y, coord.z, 'press_start')	
					end
				end
			...
			if isInMarker and not HasAlreadyEnteredMarker or (isInMarker and (LastHospital ~= currentHospital or LastPart ~= currentPart or LastPartNum ~= currentPartNum)) then
				...
				then
					TriggerEvent('dg-ambulancejob:hasExitedMarker', LastHospital, LastPart, LastPartNum)
					hasExited = true
				end

				HasAlreadyEnteredMarker, LastHospital, LastPart, LastPartNum = true, currentHospital, currentPart, currentPartNum

				TriggerEvent('dg-ambulancejob:hasEnteredMarker', currentHospital, currentPart, currentPartNum)
...
AddEventHandler('dg-ambulancejob:hasEnteredMarker', function(hospital, part, partNum)
	if DGCore.PlayerData.job and DGCore.PlayerData.job.name == 'ambulance' then
...
AddEventHandler('dg-ambulancejob:hasExitedMarker', function(hospital, part, partNum)
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction then
			DGCore.ShowHelpNotification(CurrentActionMsg)
		...
		elseif DGCore.PlayerData.job ~= nil and DGCore.PlayerData.job.name == 'ambulance' and not IsDead then
...
RegisterNetEvent('dg-ambulancejob:putInVehicle')
AddEventHandler('dg-ambulancejob:putInVehicle', function()
	local playerPed = PlayerPedId()
...
RegisterNetEvent('dg-ambulancejob:pullOutVehicle')
AddEventHandler('dg-ambulancejob:pullOutVehicle', function()
	local playerPed = PlayerPedId()
...
RegisterNetEvent('dg-ambulancejob:drag')
AddEventHandler('dg-ambulancejob:drag', function(cop)
	IsDragged = not IsDragged
	CopPed = tonumber(cop)
end)

RegisterNetEvent('dg-ambulancejob:un_drag')
AddEventHandler('dg-ambulancejob:un_drag', function(cop)
	IsDragged = not IsDragged
...
function OpenCloakroomMenu()
	DGCore.UI.Menu.Open('default', GetCurrentResourceName(), 'cloakroom', {
		title    = _U('cloakroom'),
		align    = 'bottom-right',
		elements = {
		...
		if data.current.value == 'citizen_wear' then
			DGCore.TriggerServerCallback('dg-skin:getPlayerSkin', function(skin, jobSkin)
...
function OpenVehicleSpawnerMenu(hospital, partNum)
	...
	DGCore.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle', {
		...
		if data.current.action == 'buy_vehicle' then
			...
			local authorizedVehicles = Config.AuthorizedVehicles[DGCore.PlayerData.job.grade_name]

			if #authorizedVehicles > 0 then
				for k,vehicle in ipairs(authorizedVehicles) do
					table.insert(shopElements, {
						label = ('%s - <span style="color:green;">%s</span>'):format(vehicle.label, _U('shop_item', DGCore.Math.GroupDigits(vehicle.price))),
						name  = vehicle.label,
						model = vehicle.model,
						price = vehicle.price,
						type  = 'car'
		...
		elseif data.current.action == 'garage' then
			...
			DGCore.TriggerServerCallback('dg-vehicleshop:retrieveJobVehicles', function(jobVehicles)
					...
					DGCore.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_garage', {
						...
						if data2.current.stored then
							...
							if foundSpawn then
								...
								DGCore.Game.SpawnVehicle(data2.current.model, spawnPoint.coords, spawnPoint.heading, function(vehicle)
									DGCore.Game.SetVehicleProperties(vehicle, data2.current.vehicleProps)

									TriggerServerEvent('dg-vehicleshop:setJobVehicleState', data2.current.vehicleProps.plate, false)
...
function StoreNearbyVehicle(playerCoords)
	local vehicles, vehiclePlates = DGCore.Game.GetVehiclesInArea(playerCoords, 30.0), {}
		...
		for k,v in ipairs(vehicles) do
			...
			if GetVehicleNumberOfPassengers(v) == 0 and IsVehicleSeatFree(v, -1) then
				table.insert(vehiclePlates, {
					vehicle = v,
					plate = DGCore.Math.Trim(GetVehicleNumberPlateText(v))
				})
			end
		end
	else
	...
	DGCore.TriggerServerCallback('dg-ambulancejob:storeNearbyVehicle', function(storeSuccess, foundNum)
		if storeSuccess then
			local vehicleId = vehiclePlates[foundNum]
			local attempts = 0
			DGCore.Game.DeleteVehicle(vehicleId.vehicle)
			IsBusy = true
			...
			while DoesEntityExist(vehicleId.vehicle) do
				...
				vehicles = DGCore.Game.GetVehiclesInArea(playerCoords, 30.0)
				if #vehicles > 0 then
					for k,v in ipairs(vehicles) do
						if DGCore.Math.Trim(GetVehicleNumberPlateText(v)) == vehicleId.plate then
							DGCore.Game.DeleteVehicle(v)
							break
						end
					end
				end
...
function GetAvailableVehicleSpawnPoint(hospital, part, partNum)
	...
	for i=1, #spawnPoints, 1 do
		if DGCore.Game.IsSpawnPointClear(spawnPoints[i].coords, spawnPoints[i].radius) then
...
function OpenHelicopterSpawnerMenu(hospital, partNum)
	...
	DGCore.UI.Menu.Open('default', GetCurrentResourceName(), 'helicopter_spawner', {
		...
		if data.current.action == 'buy_helicopter' then
			...
			local authorizedHelicopters = Config.AuthorizedHelicopters[DGCore.PlayerData.job.grade_name]

			if #authorizedHelicopters > 0 then
				for k,helicopter in ipairs(authorizedHelicopters) do
					table.insert(shopElements, {
						label = ('%s - <span style="color:green;">%s</span>'):format(helicopter.label, _U('shop_item', DGCore.Math.GroupDigits(helicopter.price))),
		...
		elseif data.current.action == 'garage' then
			...
			DGCore.TriggerServerCallback('dg-vehicleshop:retrieveJobVehicles', function(jobVehicles)
				if #jobVehicles > 0 then
					...
					DGCore.UI.Menu.Open('default', GetCurrentResourceName(), 'helicopter_garage', {
...
function OpenShopMenu(elements, restoreCoords, shopCoords)
	...
	DGCore.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop', {
		title    = _U('vehicleshop_title'),
		align    = 'bottom-right',
		elements = elements
	}, function(data, menu)

		DGCore.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop_confirm', {
			...
			if data2.current.value == 'yes' then
				...
				DGCore.TriggerServerCallback('dg-ambulancejob:buyJobVehicle', function (bought)
					if bought then
						DGCore.ShowNotification(_U('vehicleshop_bought', data.current.name, DGCore.Math.GroupDigits(data.current.price)))
						...
						DGCore.Game.Teleport(playerPed, restoreCoords)
					else
		...
		DGCore.Game.SpawnLocalVehicle(data.current.model, shopCoords, 0.0, function(vehicle)
	...
	DGCore.Game.SpawnLocalVehicle(elements[1].model, shopCoords, 0.0, function(vehicle)
		table.insert(spawnedVehicles, vehicle)
		TaskWarpPedIntoVehicle(playerPed, vehicle, -1)
		FreezeEntityPosition(vehicle, true)
	end)
...
function WarpPedInClosestVehicle(ped)
	...
	local vehicle, distance = DGCore.Game.GetClosestVehicle(coords)
...
RegisterNetEvent('dg-ambulancejob:heal')
AddEventHandler('dg-ambulancejob:heal', function(healType)
    local playerPed = PlayerPedId()
