...
AddEventHandler('dg:playerLoaded', function(source)
    TriggerClientEvent('dg_lumberjack:loadSpot', source, MiningPos, WashingPos, SmeltingPos)
end)

-- Refresh Spot Status:
RegisterServerEvent('dg_lumberjack:refreshSpots')
AddEventHandler('dg_lumberjack:refreshSpots', function()
    TriggerClientEvent('dg_lumberjack:loadSpot', -1, MiningPos, WashingPos, SmeltingPos)
end)

-- Update Spot Status:
RegisterServerEvent('dg_lumberjack:spotStatus')
AddEventHandler('dg_lumberjack:spotStatus', function(id, status)
    local xPlayer = DGCore.GetPlayerFromId(source)
    if MiningPos[id].pairs ~= nil then
        MiningPos[MiningPos[id].pairs].mining=status
        TriggerClientEvent('dg_lumberjack:updateStatus', -1, MiningPos[id].pairs, status)
    end
    MiningPos[id].mining=status
    TriggerClientEvent('dg_lumberjack:updateStatus', -1, id, status)
end)

-- Server Callback to get inventory pickaxe:
-- DGCore.RegisterServerCallback("dg_lumberjack:getPickaxe",function(source,cb)
-- 	local xPlayer = DGCore.GetPlayerFromId(source)
...
-- Server Callback to get inventory pickaxe:
RegisterNetEvent("dg_lumberjack:getStoneLimit")
AddEventHandler("dg_lumberjack:getStoneLimit",function()
	local xPlayer = DGCore.GetPlayerFromId(source)
	local player = xPlayer.identifier
	table.insert(didmining, {identifier = player})
end)

DGCore.RegisterServerCallback("dg_lumberjack:didmining",function(source, cb) 
local xPlayer = DGCore.GetPlayerFromId(source)
local did = 0
...
-- DGCore.RegisterServerCallback("dg_lumberjack:getWashPan",function(source,cb)
-- 	local xPlayer = DGCore.GetPlayerFromId(source)
...
-- -- Server Callback to get inventory pickaxe:
-- DGCore.RegisterServerCallback("dg_lumberjack:getWashedStoneLimit",function(source,cb)
-- 	local xPlayer = DGCore.GetPlayerFromId(source)
...
-- DGCore.RegisterServerCallback("dg_lumberjack:getWashedStone",function(source,cb)
-- 	local xPlayer = DGCore.GetPlayerFromId(source)
...
-- DGCore.RegisterServerCallback("dg_lumberjack:removeStone",function(source,cb)
-- 	local xPlayer = DGCore.GetPlayerFromId(source)
...
-- RegisterServerEvent("dg_lumberjack:reward")
-- AddEventHandler("dg_lumberjack:reward", function(itemName,itemAmount)
-- 	local xPlayer = DGCore.GetPlayerFromId(source)
-- 	local itemLabel = DGCore.GetItemLabel(itemName)
...
-- RegisterServerEvent("dg_lumberjack:rewardSmelting")
-- AddEventHandler("dg_lumberjack:rewardSmelting", function()
-- 	local xPlayer = DGCore.GetPlayerFromId(source)
