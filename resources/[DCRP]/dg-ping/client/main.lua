    ...
    Citizen.CreateThread(function()
        ...
        while isPending do
            ...
            if count >= Config.Timeout and isPending then
                ...
                TriggerServerEvent('dg-ping:server:SendPingResult', pendingPing.id, 'timeout')
                pendingPing = nil
                isPending = false
            end
            Citizen.Wait(1000)
...
RegisterNetEvent('dg-ping:client:SendPing')
AddEventHandler('dg-ping:client:SendPing', function(sender, senderId)
    if pendingPing == nil then
        ...
        TriggerServerEvent('dg-ping:server:SendPingResult', pendingPing.id, 'received')
        ...
        TriggerServerEvent('dg-ping:server:SendPingResult', senderId, 'unable')
    end
end)

RegisterNetEvent('dg-ping:client:AcceptPing')
AddEventHandler('dg-ping:client:AcceptPing', function()
    if isPending then
        ...
        TriggerServerEvent('dg-ping:server:SendPingResult', pendingPing.id, 'accept')
        isPending = false
    else
...
RegisterNetEvent('dg-ping:client:RejectPing')
AddEventHandler('dg-ping:client:RejectPing', function()
    if pendingPing ~= nil then
        TriggerEvent('DoLongHudText', 'Rejected Ping From ' .. pendingPing.name, 2)
        TriggerServerEvent('dg-ping:server:SendPingResult', pendingPing.id, 'reject')
        pendingPing = nil
        isPending = false
    else
        TriggerEvent('DoLongHudText', 'You Have No Pending Ping', 2)
...
RegisterNetEvent('dg-ping:client:RemovePing')
AddEventHandler('dg-ping:client:RemovePing', function()
    if pendingPing ~= nil then
        RemoveBlip(pendingPing.blip)
        pendingPing = nil
