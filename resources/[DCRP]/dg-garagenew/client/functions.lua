...
SpawnLocalVehicle = function(data)
	...
	if not DGCore.Game.IsSpawnPointClear(spawnpoint["position"], 3.0) then 
	...
	DGCore.Game.SpawnLocalVehicle(vehicleProps["model"], spawnpoint["position"], spawnpoint["heading"], function(yourVehicle)
...
SpawnVehicle = function(data,recuperar)
	...
	DGCore.Game.SpawnVehicle(vehicleProps["model"], spawnpoint["position"], spawnpoint["heading"], function(yourVehicle)
		SetVehicleProperties(yourVehicle, vehicleProps)
...
PutInVehicle = function()
    ...
    if DoesEntityExist(vehicle) then
		...
		DGCore.TriggerServerCallback("betrayed_garage:validateVehicle", function(valid)
			if valid then
				AbrirMenuGuardar()
            else
                CloseMenu()
...
SetVehicleProperties = function(vehicle, vehicleProps)
    DGCore.Game.SetVehicleProperties(vehicle, vehicleProps)
