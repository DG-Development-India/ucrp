...
SpawnLocalVehicle = function(data)
	...
	if not DGCoreGame.IsSpawnPointClear(spawnpoint["position"], 3.0) then 
	...
	DGCoreGame.SpawnLocalVehicle(vehicleProps["model"], spawnpoint["position"], spawnpoint["heading"], function(yourVehicle)
...
SpawnVehicle = function(data,recuperar)
	...
	CloseMenu()
	local gameVehicles = DGCoreGame.GetVehicles()
	...
	DGCoreGame.SpawnVehicle(vehicleProps["model"], spawnpoint["position"], spawnpoint["heading"], function(yourVehicle)
		SetVehicleProperties(yourVehicle, vehicleProps)
...
PutInVehicle = function()
    ...
    if DoesEntityExist(vehicle) then
		...
		DGCoreTriggerServerCallback("erp_garage:validateVehicle", function(valid)
			if valid then
				AbrirMenuGuardar()
            else
                CloseMenu()
...
SetVehicleProperties = function(vehicle, vehicleProps)
    DGCoreGame.SetVehicleProperties(vehicle, vehicleProps)
...
GetVehicleProperties = function(vehicle)
    if DoesEntityExist(vehicle) then
        local vehicleProps = DGCoreGame.GetVehicleProperties(vehicle)
...
DrawScriptMarker = function(markerData)
    DrawMarker(20, markerData["pos"],0,0,0,0,0,0,0.701,1.0001,0.3001,222, 50, 50, 0.05,0, 0,0,0,0,0,0)
      --  DGCoreShowHelpNotification("Press [E] to Access Garage")
end
