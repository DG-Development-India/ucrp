...
AddEventHandler("mech:tools", function(args)
    ...
    elseif args[1] == "add" then
        if exports["dg-inventory"]:hasEnoughOfItem(args[2],1,false) then
...
Citizen.CreateThread(function()
    while true do
        ...
        TriggerEvent('chat:addSuggestion', '/mech remove', '/mech remove [materials] [amount]')
    ...
    if DGCore.PlayerData.job and DGCore.PlayerData.job.name == "mechanic" then
        ...
        if not notfucked then
            ...
            TaskPlayAnim(playerped,"mp_car_bomb","car_bomb_mechanic",8.0, -8, -1, 49, 0, 0, 0, 0)
                ...
                local finished = exports["dg-taskbar"]:taskBar(15000,"Repairing")
