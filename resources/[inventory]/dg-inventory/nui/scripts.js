...
$(document).ready(function () {
    $('.save-settings').click(() => {
        ...
        $.post(
            'https://dg-inventory/UpdateSettings',
            JSON.stringify({
...
function invStack(
    ...
    $.post('https://dg-inventory/stack', JSON.stringify(arr));
}
...
function invMove(
    ...
    $.post('https://dg-inventory/move', JSON.stringify(arr));
}
...
function invSwap(targetSlot, targetInventory, originSlot, originInventory, itemid1, metainformation1, itemid2, metainformation2) {
    ...
    $.post('https://dg-inventory/swap', JSON.stringify(arr));
}

function removeCraftItems(itemid, moveAmount) {
    let arr = itemList[itemid].craft;
    let amount = moveAmount;
    $.post('https://dg-inventory/removeCraftItems', JSON.stringify([arr, amount]));
}
...
function DisplayInventoryMultiple(playerinventory, itemCount, invName, targetinventory, targetitemCount, targetinvName, cash, Owner, targetInvWeight = 0) {
    ...
    if (targetinvName.indexOf('Drop') > -1) {
        ...
        slotLimitTarget = 200;
        displayName = 'Stash';
    } else if (targetinvName.indexOf('Briefcase') > -1) {
        secondaryMaxWeight = 1000.0;
        slotLimitTarget = 20;
        ...
        slotLimitTarget = 5;
    } else if (targetinvName.indexOf("Stash-Recycle") > -1) {
        secondaryMaxWeight = 100000.0;
        slotLimitTarget = 100;
    ...
    } else if (targetinvName.indexOf('MT-') > -1) {
        displayName = 'Motel';
        secondaryMaxWeight = 7500.0;
        slotLimitTarget = 100;
    } else if (targetinvName.indexOf('APT-') > -1) {
        displayName = 'Apartment';
        secondaryMaxWeight = 15000.0;
        slotLimitTarget = 100;
    } else if (targetinvName.indexOf(':') > -1) {
...
function BuildDrop(brokenSlots) {
    $.post(
        'https://dg-inventory/dropIncorrectItems',
        JSON.stringify({
            slots: brokenSlots,
        }),
    );
...
$(document).on('mousedown mouseup', (event) => {
    ...
    if (element.id === 'CloseInv') {
        ...
        if (draggingid.includes('secondary')) {
            isDragging = false;
            draggingid = 'none';
            EndDragError(element)
            InventoryLog("Error: You can't use that item from there!")
            return;
        }
        useitem();
        return;
...
function DragToggle(fromSlot, isUsing, mouseEvent) {
    ...
    if (fromSlot) {
        ...
        if (occupiedslot && !isDragging) {
            ...
            $.post('https://dg-inventory/SlotInuse', JSON.stringify(parseInt(draggingid.replace(/\D/g, ''))));
            ...
            if (isUsing || (TargetInventoryName === 'Craft' && fromSlot.includes('player'))) {
                if (fromSlot.includes('secondary')){
                    dragging = false;
                    draggingid = "none";
                    EndDragError(fromSlot);
                    InventoryLog("Error: You can't use that item from there!")
                }
                else {
                    return;
            ...
            if ((mouseEvent.shiftKey || mouseEvent.ctrlKey) && shopOrCraft && fromSlot.includes('secondary')) {
                if (moveAmount > 0)
                {
                    ...
                    draggingid = "none";
                    EndDragError(fromSlot);
                    InventoryLog("Error: Please enter valid amount of item!")
                }
                return;
            }
...
function DropItem(slot, amountDropped) {
    ...
    // $.post('https://dg-inventory/dropitem', JSON.stringify({
...
function ErrorMove() {
    // $.post('https://dg-inventory/move:fail', JSON.stringify({}));
}

function SuccessMove() {
    // $.post('https://dg-inventory/move:success', JSON.stringify({}));
}
...
function CompileStacks(
    ...
    $.post(
        'https://dg-inventory/SlotJustUsed',
        JSON.stringify({
    ...
    InventoryLog("Moved " + moveAmount + "x of " + itemidsent + " from " + originInventory + " [" + originSlot + "] to " + targetInventory + " [" + targetSlot + "]")
    if (crafting) {
...
function MoveStack(targetSlot, originSlot, inv1, inv2, purchase, itemCosts, itemidsent, metainformation, moveAmount) {
    ...
    InventoryLog("Moved all of " + itemidsent + " from " + originInventory + " [" + originSlot + "] to " + targetInventory + " [" + targetSlot + "]")
    if (crafting) {
        removeCraftItems(itemidsent, moveAmount);
        closeInv()
        return;
...
function SwapStacks(targetSlot, originSlot, inv1, inv2, itemid1, metainformation1, itemid2, metainformation2) {
    // $.post('https://dg-inventory/swapstack', JSON.stringify({
    //   slot1: slot1,
    //   slot2: slot2,
    //   inv1: inv1,
    //   inv2: inv2,
    ...
    InventoryLog("Swapped " + itemid1 + " from " + originInventory + " [" + originSlot + "] with " + itemid2 + " from " + targetInventory + " [" + targetSlot + "]")
    UpdateSetWeights('');
}
...
//     $.post(
//         'https://dg-inventory/ServerCloseInventory',
//         JSON.stringify({
...
function closeInv(pIsItemUsed = false) {
    ...
    $.post("http://dg-inventory/ServerCloseInventory", JSON.stringify({
      name: TargetInventoryName
    }));
    TargetInventoryName = "none";
    $.post("http://dg-inventory/Close", JSON.stringify({
      isItemUsed: pIsItemUsed
    }));
}
    ...
    if (result == 'Success' && result2 == 'Success') {
        ...
        if (stacking) {
            ...
            if (inventoryDropName == 'Shop' || inventoryDropName == 'Craft' || (!StoreOwner && PlayerStore)) {
                ...
                if (inventoryDropName == inventoryReturnItemDropName) {
                    SwapStacks(parseInt(slot.replace(/\D/g, '')),parseInt(draggingid.replace(/\D/g, '')),inventoryDropName,inventoryReturnItemDropName,itemid1,metainformation1,itemid2,metainformation2);
                    ...
                    InventoryLog("Error: You can't swap to another inventory")
                }
            }
        }
...
function useitem() {
    ...
    if (inventoryUsedName == PlayerInventoryName) {
        ...
        $.post('https://dg-inventory/invuse', JSON.stringify(arr));
