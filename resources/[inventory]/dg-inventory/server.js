...
function DeleteDropInRestart() {
    ...
    let string3 = `DELETE FROM user_inventory2 WHERE name LIKE 'hidden%'`
    exports.ghmattimysql.execute(string3,{});
    let string4 = `DELETE FROM user_inventory2 WHERE name LIKE 'Trash%'`
    exports.ghmattimysql.execute(string4,{});
    let string2 = `DELETE FROM inventory_stash WHERE name LIKE 'Stash-MRPD-Evidence-Locker'`
    exports.ghmattimysql.execute(string2,{});
}
...
onNet("server-request-update", async (player) => {
    let src = source
    let playerinvname = player
    let string = `SELECT count(item_id) as amount, id, item_id, name, information, slot, dropped FROM user_inventory2 WHERE name = '${player}' group by item_id`; 
    ...
    onNet("server-inventory-open", async ( coords, player, secondInventory, targetName, itemToDropArray, sauce) => {
        ...
        exports.ghmattimysql.execute(string,{}, function(inventory) {
            ...
            if(secondInventory == "1") {
                ...
                if (targetinvname.match(/^Trunk-.*$/)) {
                    string = `SELECT count(item_id) as amount, id, name, item_id, information, slot, dropped, quality, creationDate FROM inventory_trunk WHERE name = '${targetinvname}' group by slot`; 
                }
                else if (targetinvname.match(/^MT-.*$/)) {
                    string = `SELECT count(item_id) as amount, id, name, item_id, information, slot, dropped, quality, creationDate FROM inventory_motel WHERE name = '${targetinvname}' group by slot`; 
                }
                else if (targetinvname.match(/^APT-.*$/)) {
                    string = `SELECT count(item_id) as amount, id, name, item_id, information, slot, dropped, quality, creationDate FROM inventory_motel WHERE name = '${targetinvname}' group by slot`; 
                }
                else if (targetinvname.match(/^Stash-.*$/)) {
                    string = `SELECT count(item_id) as amount, id, name, item_id, information, slot, dropped, quality, creationDate FROM inventory_stash WHERE name = '${targetinvname}' group by slot`; 
                }
...
else if(secondInventory == "83")
{
    var targetinvname = targetName;
    var shopArray = burgershotcraft();
    var shopAmount = 6;
    emitNet("inventory-open-target", src, [invArray,arrayCount,playerinvname,shopArray,shopAmount,targetinvname,500,false]);
}

else if(secondInventory == "84")
{
    var targetinvname = targetName;
    var shopArray = GroceryShop();
    var shopAmount = 8;
    emitNet("inventory-open-target", src, [invArray,arrayCount,playerinvname,shopArray,shopAmount,targetinvname,500,false]);
}

else if(secondInventory == "85")
{
    var targetinvname = targetName;
    var shopArray = VegetableShop();
    var shopAmount = 12;
    emitNet("inventory-open-target", src, [invArray,arrayCount,playerinvname,shopArray,shopAmount,targetinvname,500,false]);
}
...
// Gun Crafting
else if(secondInventory == "90")
{
    var targetinvname = targetName;
    var shopArray = BallasGunComponent();
    var shopAmount = 27;
    emitNet("inventory-open-target", src, [invArray,arrayCount,playerinvname,shopArray,shopAmount,targetinvname,500,false]);
}

else if(secondInventory == "91")
{
    var targetinvname = targetName;
    var shopArray = CartelGunComponent();
    var shopAmount = 27;
    emitNet("inventory-open-target", src, [invArray,arrayCount,playerinvname,shopArray,shopAmount,targetinvname,500,false]);
}
...
else if(secondInventory == "92")
{
    var targetinvname = targetName;
    var shopArray = BallasGunCraft();
    var shopAmount = 6;
    emitNet("inventory-open-target", src, [invArray,arrayCount,playerinvname,shopArray,shopAmount,targetinvname,500,false]);
}

else if(secondInventory == "93")
{
    var targetinvname = targetName;
    var shopArray = CartelGunCraft();
    var shopAmount = 6;
    emitNet("inventory-open-target", src, [invArray,arrayCount,playerinvname,shopArray,shopAmount,targetinvname,500,false]);
}

else if(secondInventory == "94")
{
    var targetinvname = targetName;
    var shopArray = MWGunComponent();
    var shopAmount = 23;
    emitNet("inventory-open-target", src, [invArray,arrayCount,playerinvname,shopArray,shopAmount,targetinvname,500,false]);
}

else if(secondInventory == "95")
{
    var targetinvname = targetName;
    var shopArray = MWGunCraft();
    var shopAmount = 5;
    emitNet("inventory-open-target", src, [invArray,arrayCount,playerinvname,shopArray,shopAmount,targetinvname,500,false]);
}

else if(secondInventory == "96")
{
    var targetinvname = targetName;
    var shopArray = BestBudsShop();
    var shopAmount = 11;
    emitNet("inventory-open-target", src, [invArray,arrayCount,playerinvname,shopArray,shopAmount,targetinvname,500,false]);
}

else if(secondInventory == "97")
{
    var targetinvname = targetName;
    var shopArray = BestBudsCraft();
    var shopAmount = 4;
    emitNet("inventory-open-target", src, [invArray,arrayCount,playerinvname,shopArray,shopAmount,targetinvname,500,false]);
}

else if(secondInventory == "98")
{
    var targetinvname = targetName;
    var shopArray = NexusAttachments();
    var shopAmount = 7;
    emitNet("inventory-open-target", src, [invArray,arrayCount,playerinvname,shopArray,shopAmount,targetinvname,500,false]);
}
...
{
    var targetinvname = targetName;
    var shopArray = tacoshop();
    var shopAmount = 8;
    emitNet("inventory-open-target", src, [invArray,arrayCount,playerinvname,shopArray,shopAmount,targetinvname,500,false]);
}

else if(secondInventory == "106")
{
    var targetinvname = targetName;
    var shopArray = mechaniccrafting2();
    var shopAmount = 4;
    emitNet("inventory-open-target", src, [invArray,arrayCount,playerinvname,shopArray,shopAmount,targetinvname,500,false]);
}
...
onNet("server-inventory-move", async (player, data, coords) => {
    ...
    let moveAmount = data[7]
    let crafting = data[8]
    let isWeapon = data[9]
    let PlayerStore = data[10]
    let creationDate = Date.now()
    
    var istTrunk, istMotel, istStash
    var isoTrunk, isoMotel, isoStash

    if (targetName.match(/^Trunk-.*$/)) { istTrunk = true }
    else if (targetName.match(/^MT-.*$/)) {  istMotel = true }
    else if (targetName.match(/^APT-.*$/)) {  istMotel = true }
    else if (targetName.match(/^Stash-.*$/)) {  istStash = true }

    if (startname.match(/^Trunk-.*$/)) { isoTrunk = true }
    else if (startname.match(/^MT-.*$/)) {  isoMotel = true }
    else if (startname.match(/^APT-.*$/)) {  isoMotel = true }
    else if (startname.match(/^Stash-.*$/)) {  isoStash = true }
    ...
    if (purchase) {
        ...
        if (!PlayerStore) {
            ...
            if (targetName.indexOf("Drop") > -1 || targetName.indexOf("hidden") > -1) {
                ...
                if (startname == targetName) {
                    if (istTrunk) {
                        db(`UPDATE inventory_trunk SET slot='${targetslot}', name='${targetName}', dropped='0' WHERE slot='${startslot}' and name='${startname}';`); 
                    }
                    else if (istMotel) {
                        db(`UPDATE inventory_motel SET slot='${targetslot}', name='${targetName}', dropped='0' WHERE slot='${startslot}' and name='${startname}';`); 
                    }
                    else if (istStash) {
                        db(`UPDATE inventory_stash SET slot='${targetslot}', name='${targetName}', dropped='0' WHERE slot='${startslot}' and name='${startname}';`); 
                    }
                    else {
                    ...
                    if (istTrunk) {
                        db(`DELETE FROM user_inventory2 WHERE name = '${startname}' and slot='${startslot}';`)
                        for (let i = 0; i < parseInt(moveAmount); i++) {
                            db(`INSERT INTO inventory_trunk (item_id, name, information, slot, creationDate) VALUES ('${itemidsent}','${targetName}','${info}','${targetslot}','${creationDate}' );`);
                        }
                    }
                    else if (istMotel) {
                        db(`DELETE FROM user_inventory2 WHERE name = '${startname}' and slot='${startslot}';`)
                        for (let i = 0; i < parseInt(moveAmount); i++) {
                            db(`INSERT INTO inventory_motel (item_id, name, information, slot, creationDate) VALUES ('${itemidsent}','${targetName}','${info}','${targetslot}','${creationDate}' );`);
                        }
                    }
                    else if (istStash) {
                        db(`DELETE FROM user_inventory2 WHERE name = '${startname}' and slot='${startslot}';`)
                        for (let i = 0; i < parseInt(moveAmount); i++) {
                            db(`INSERT INTO inventory_stash (item_id, name, information, slot, creationDate) VALUES ('${itemidsent}','${targetName}','${info}','${targetslot}','${creationDate}' );`);
                        }
                    }
                    else {
                        if (isoTrunk) {
                            db(`DELETE FROM inventory_trunk WHERE name = '${startname}' and slot='${startslot}';`)
                            for (let i = 0; i < parseInt(moveAmount); i++) {
                                db(`INSERT INTO user_inventory2 (item_id, name, information, slot, creationDate) VALUES ('${itemidsent}','${targetName}','${info}','${targetslot}','${creationDate}' );`);
                            }
                        }
                        else if (isoMotel) {
                            db(`DELETE FROM inventory_motel WHERE name = '${startname}' and slot='${startslot}';`)
                            for (let i = 0; i < parseInt(moveAmount); i++) {
                                db(`INSERT INTO user_inventory2 (item_id, name, information, slot, creationDate) VALUES ('${itemidsent}','${targetName}','${info}','${targetslot}','${creationDate}' );`);
                            }
                        }
                        else if (isoStash) {
                            db(`DELETE FROM inventory_stash WHERE name = '${startname}' and slot='${startslot}' LIMIT ${moveAmount};`)
                            for (let i = 0; i < parseInt(moveAmount); i++) {
                                db(`INSERT INTO user_inventory2 (item_id, name, information, slot, creationDate) VALUES ('${itemidsent}','${targetName}','${info}','${targetslot}','${creationDate}' );`);
                            }
                        }
        ...
        // if (isWeapon !== false) {
        //     emitNet('ped-unarmed', -1, itemidsent)
        // }
        if (crafting == true) {
            info - await GenerateInformation(player,itemidsent)
            for (let i = 0; i < parseInt(moveAmount); i++) {
...
onNet("server-inventory-stack", async (player, data, coords) => {
    ...
    if (originInventory.match(/^Trunk-.*$/)) { isoTrunk = true }
    else if (originInventory.match(/^MT-.*$/)) {  isoMotel = true }
    else if (originInventory.match(/^APT-.*$/)) {  isoMotel = true }
    else if (originInventory.match(/^Stash-.*$/)) {  isoStash = true }
    ...
    if (purchase) {
        ...
        if (targetName.indexOf("Drop") > -1 || targetName.indexOf("hidden") > -1) {
            db(`UPDATE user_inventory2 SET slot='${targetslot}', name='${targetName}', dropped='0' WHERE slot='${originSlot}' and name='${originInventory}' and item_id='${itemidsent}' LIMIT ${moveAmount}`);
        }

        else {

            if (originInventory == targetName) {
                if (istTrunk) {
                    db(`UPDATE inventory_trunk SET slot='${targetslot}', name='${targetName}', dropped='0' WHERE slot='${originSlot}' and name='${originInventory}' and item_id='${itemidsent}' LIMIT ${moveAmount}`); 
                }
                else if (istMotel) {
                    db(`UPDATE inventory_motel SET slot='${targetslot}', name='${targetName}', dropped='0' WHERE slot='${originSlot}' and name='${originInventory}' and item_id='${itemidsent}' LIMIT ${moveAmount}`); 
                }
                else if (istStash) {
                    db(`UPDATE inventory_stash SET slot='${targetslot}', name='${targetName}', dropped='0' WHERE slot='${originSlot}' and name='${originInventory}' and item_id='${itemidsent}' LIMIT ${moveAmount}`); 
                }
                else {
                    db(`UPDATE user_inventory2 SET slot='${targetslot}', name='${targetName}', dropped='0' WHERE slot='${originSlot}' and name='${originInventory}' and item_id='${itemidsent}' LIMIT ${moveAmount}`);
                }
            ...
            {
                if (istTrunk) {
                    db(`DELETE FROM user_inventory2 WHERE name = '${originInventory}' and slot='${originSlot}' LIMIT ${moveAmount};`)
                    for (let i = 0; i < parseInt(moveAmount); i++) {
                        db(`INSERT INTO inventory_trunk (item_id, name, information, slot, creationDate) VALUES ('${itemidsent}','${targetName}','${info}','${targetslot}','${creationDate}' );`); 
                    }
                }
                ...
                {
                    if (isoTrunk) {
                        db(`DELETE FROM inventory_trunk WHERE name = '${originInventory}' and slot='${originSlot}' LIMIT ${moveAmount};`)
                        for (let i = 0; i < parseInt(moveAmount); i++) {
                            db(`INSERT INTO user_inventory2 (item_id, name, information, slot, creationDate) VALUES ('${itemidsent}','${targetName}','${info}','${targetslot}','${creationDate}' );`); 
                        }
                    }
                    else if (isoMotel) {
                        db(`DELETE FROM inventory_motel WHERE name = '${originInventory}' and slot='${originSlot}' LIMIT ${moveAmount};`)
                        for (let i = 0; i < parseInt(moveAmount); i++) {
                            db(`INSERT INTO user_inventory2 (item_id, name, information, slot, creationDate) VALUES ('${itemidsent}','${targetName}','${info}','${targetslot}','${creationDate}' );`); 
                        }
                    }
                    else if (isoStash) {
                        db(`DELETE FROM inventory_stash WHERE name = '${originInventory}' and slot='${originSlot}' LIMIT ${moveAmount};`)
                        for (let i = 0; i < parseInt(moveAmount); i++) {
                            db(`INSERT INTO user_inventory2 (item_id, name, information, slot, creationDate) VALUES ('${itemidsent}','${targetName}','${info}','${targetslot}','${creationDate}' );`); 
                        }
                    }
...
onNet("server-inventory-swap", (player, data, coords) => {
    ...
    if (targetName ==  originInventory)
    {

        if (istTrunk) 
        {
            let string = `SELECT item_id FROM inventory_trunk WHERE slot='${targetSlot}' and name='${targetName}'`;
            let string2 = `SELECT item_id FROM inventory_trunk WHERE slot='${originSlot}' and name='${originInventory}'`;

            exports.ghmattimysql.execute(string,{}, function(result) {
                let string2 = `UPDATE inventory_trunk SET slot='${originSlot}', name='${originInventory}', dropped='0' WHERE slot='${targetSlot}' and name='${targetName}' and item_id='${result[0].item_id}'`
                exports.ghmattimysql.execute(string2,{}, function(result) {
                })
            });

            exports.ghmattimysql.execute(string2,{}, function(result) {
            let string3 = `UPDATE inventory_trunk SET slot='${targetSlot}', name='${targetName}', dropped='0' WHERE slot='${originSlot}' and name='${originInventory}' and item_id='${result[0].item_id}'`
        ...
        {
            let string = `SELECT item_id FROM inventory_motel WHERE slot='${targetSlot}' and name='${targetName}'`;
            let string2 = `SELECT item_id FROM inventory_motel WHERE slot='${originSlot}' and name='${originInventory}'`;

            exports.ghmattimysql.execute(string,{}, function(result) {
                let string2 = `UPDATE inventory_motel SET slot='${originSlot}', name='${originInventory}', dropped='0' WHERE slot='${targetSlot}' and name='${targetName}' and item_id='${result[0].item_id}'`
                exports.ghmattimysql.execute(string2,{}, function(result) {
                })
            });

            exports.ghmattimysql.execute(string2,{}, function(result) {
            let string3 = `UPDATE inventory_motel SET slot='${targetSlot}', name='${targetName}', dropped='0' WHERE slot='${originSlot}' and name='${originInventory}' and item_id='${result[0].item_id}'`
                exports.ghmattimysql.execute(string3,{}, function(result) {
        ...
        {
            let string = `SELECT item_id FROM inventory_stash WHERE slot='${targetSlot}' and name='${targetName}'`;
            let string2 = `SELECT item_id FROM inventory_stash WHERE slot='${originSlot}' and name='${originInventory}'`;

            exports.ghmattimysql.execute(string,{}, function(result) {
                let string2 = `UPDATE inventory_stash SET slot='${originSlot}', name='${originInventory}', dropped='0' WHERE slot='${targetSlot}' and name='${targetName}' and item_id='${result[0].item_id}'`
                exports.ghmattimysql.execute(string2,{}, function(result) {
                })
            });

            exports.ghmattimysql.execute(string2,{}, function(result) {
            let string3 = `UPDATE inventory_stash SET slot='${targetSlot}', name='${targetName}', dropped='0' WHERE slot='${originSlot}' and name='${originInventory}' and item_id='${result[0].item_id}'`
                exports.ghmattimysql.execute(string3,{}, function(result) {
