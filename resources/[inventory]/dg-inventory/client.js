...
let caseDumps = [
	{ objectID: 1049338225, Description: "Briefcase" }
];
...
function ScanCase() {
	...
	if (model !== 0) {
		for (let x in caseDumps) {
			if (x) {
				if (caseDumps[x].objectID == model) {
...
// RegisterNetEvent('ped-unarmed')
// on('ped-unarmed', (itemid) => {
// 	let weaponhash = GetSelectedPedWeapon(PlayerPedId())
// 	if (weaponhash == itemid) {
...
on('inventory-open-request', () => {
	...
	let tacoShopDst = GetDistanceBetweenCoords(startPosition[0],startPosition[1],startPosition[2],15.72802066803, -1598.2983398438, 29.377981185913)
	//let tacoShopDst1 = GetDistanceBetweenCoords(startPosition[0],startPosition[1],startPosition[2],11.69, -1599.69, 29.38)



	let isInVehicle = IsPedInAnyVehicle(PlayerPedId(), true);
	if (isInVehicle) {
	...
	else 
	{
		...
		//} else if (tacoShopDst1 < 2.0) {
			//TriggerEvent("server-inventory-open", "101", "Craft");
		} 
			...
			let x = parseInt(CaseFound[0]);
			let y = parseInt(CaseFound[1]);
			let container = "Briefcase|" + x + "|" + y;
			emitNet("server-inventory-open", startPosition, cid, "1", container);
		}
...
function drawMarkersUI() {
	for (let Row in DrawInventories) {		
		DrawMarker(20,DrawInventories[Row].position.x, DrawInventories[Row].position.y, DrawInventories[Row].position.z-0.6,0,0,0,0,0,0,0.35,0.5,0.15,252,255,255,91,0,0,0,0)
	}	
}
...
on("__cfx_nui:stack", (data,cb) => {
	...
	//emitNet('closeInventoryGui')
	// emit("server-request-update-src", cid)
	emitNet('motel:logs',data[1],data[7],'stacked to',data[4],data[3],data[2],data[0])
})
...
on("__cfx_nui:move", (data,cb) => {
	...
	//emitNet('closeInventoryGui')
	// emit("server-request-update-src", cid)
	emitNet('motel:logs',data[7],data[6],'moved from',data[3],data[1],data[2],data[0])
})
...
on("__cfx_nui:swap", (data,cb) => {	
	...
	// emit("server-request-update-src", cid)
	emitNet('motel:logs',data[1],data[7],'swapped from',data[4],data[3],data[2],data[0])
})
...
RegisterNetEvent('inventory-open-my')
on('inventory-open-my', () => {
	if (canOpen === true) {
		OpenGui()
		PopulateGui(MyInventory,MyItemCount,cid,{},0,'none',cash);
		SendNuiMessage(JSON.stringify({ response: "EnableMouse" }))
		ResetCache(true)
	}
});
...
//updated trunk animation
let lastvehicle = 0

RegisterNetEvent('toggle-animation')
on('toggle-animation', (toggleAnimation, _vehf) => {
	let lPed = PlayerPedId()
	if(toggleAnimation) {
		lastvehicle = parseInt(_vehf)
