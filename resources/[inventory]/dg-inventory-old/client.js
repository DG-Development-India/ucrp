...
RegisterNetEvent('inv:addItem')
on('inv:addItem', (id, amount,generateInformation, itemdata) => {
	if (personalWeight > 250) {
		emit("DoLongHudText","Failed to give " + id + " because you were overweight!",2)
		return
	}
...
on('inventory-open-request', () => {
	...
	if (isInVehicle) {
		...
		let vehicleFound = GetVehiclePedIsIn(PlayerPedId(),false)
...
function drawMarkersUI() {
	for (let Row in DrawInventories) {		
		DrawMarker(20,DrawInventories[Row].position.x, DrawInventories[Row].position.y, DrawInventories[Row].position.z-0.6,0,0,0,0,0,0,0.35,0.5,0.15,255,0,0,150,0,0,0,0)
	}	
}
...
on("__cfx_nui:stack", (data,cb) => {
	...
	emitNet('motel:logs',data[1],data[7],'Moved from',data[4],data[3],data[2],data[0])
})
/*
