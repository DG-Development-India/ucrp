...
AddEventHandler('RunUseItem', function(itemid, slot, inventoryName, isWeapon)
    ...
    if (itemid == "tuner") then
      ...
      local finished = exports["dg-taskbar"]:taskBar(2000,"Connecting Tuner Laptop",false,false,playerVeh)
    ...
    if (itemid == "highgradefemaleseed" ) then
        ...
        TriggerServerEvent("dg-weedplants:highgradefemaleseed",itemid)
        end
    end
    --
    ...
    if (itemid == "lsdtab" or itemid == "badlsdtab") then
        ...
        local finished = exports["dg-taskbar"]:taskBar(3000,"Placing LSD Strip on 👅",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "decryptersess" or itemid == "decrypterfv2" or itemid == "decrypterenzo") then
      if (#(GetEntityCoords(player) - vector3( 1275.49, -1710.39, 54.78)) < 3.0) then
          local finished = exports["dg-taskbar"]:taskBar(25000,"Decrypting Data",false,false,playerVeh)
          if (finished == 100) then
    ...
    if (itemid == "cr") then
        TriggerEvent("dg-doorlock:UseRedKeycard", itemid)
    end

    -- if (itemid == "weedoz") then
 
    --   local finished = exports["dg-taskbar"]:taskBar(5000,"Packing Q Bags",false,false,playerVeh)
    --     if (finished == 100) then
    ...
    if ( itemid == "weedoz" and hasEnoughOfItem("qualityscales",1,false) and hasEnoughOfItem("rollingpaper",1,false) ) then
        playAnim("missheistdockssetup1clipboard@idle_a", "idle_b", 10000)
        local finished = exports["dg-taskbar"]:taskBar(10000,"Rolling Joint",false,false,playerVeh)
        if (finished == 100) then
            --CreateCraftOption("joint", 5, true)
            TriggerEvent('inventory:removeItem', "weedoz", 1)
            TriggerEvent('inventory:removeItem', "rollingpaper", 1)
            TriggerEvent('player:receiveItem', "joint", 5) 
        end
    end

    if (itemid == "weedoz" and hasEnoughOfItem("rollingpaper",1,false) ) then
        playAnim("missheistdockssetup1clipboard@idle_a", "idle_b", 10000)
        local finished = exports["dg-taskbar"]:taskBar(10000,"Rolling Joint",false,false,playerVeh)
        if (finished == 100) then
            ...
            TriggerEvent('inventory:removeItem', "weedoz", 1)
            TriggerEvent('inventory:removeItem', "rollingpaper", 1)
            TriggerEvent('player:receiveItem', "joint", 2) 
        end
    end

    if (itemid == "lighter") then
        TriggerEvent("animation:PlayAnimation","lighter")
          local finished = exports["dg-taskbar"]:taskBar(2000,"Starting Fire",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "joint") then
        ...
        local finished = exports["dg-taskbar"]:taskBar(5000,"Smoking Joint",false,false,playerVeh)
        if (finished == 100) then
            Wait(200)
            TriggerEvent("Evidence:StateSet",3,600)
            TriggerEvent("Evidence:StateSet",4,600)
            local armor = GetPedArmour(GetPlayerPed(-1))
            SetPedArmour(GetPlayerPed(-1), armor+2)
    ...
    if (itemid == "Gruppe6Card") then
        TriggerServerEvent('dg-securityheists:banktstart')
    end


    if (itemid == "usbdevice") then
        local finished = exports["dg-taskbar"]:taskBar(15000,"Scanning For Networks",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "armor") then
        ...
        local finished = exports["dg-taskbar"]:taskBar(5000,"Armor",true,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "bodybag") then
        local finished = exports["dg-taskbar"]:taskBar(10000,"Opening bag",true,false,playerVeh)
        if (finished == 100) then
            remove = true
    ...
    if (itemid == "bodygarbagebag") then
            local finished = exports["dg-taskbar"]:taskBar(10000,"Opening trash bag",false,false,playerVeh)
            if (finished == 100) then
                remove = true
    ...
    if (itemid == "foodsupplycrate") then
        ...
        local finished = exports["dg-taskbar"]:taskBar(20000,"Opening the crate (ESC to Cancel)",false,false,playerVeh)
        if (finished == 100) then
            remove = true
    ...
    if (itemid == "organcooler") then
        local finished = exports["dg-taskbar"]:taskBar(5000,"Opening cooler",true,false,playerVeh)
        if (finished == 100) then
            remove = true
    ...
    if (itemid == "Bankbox") then
        if (hasEnoughOfItem("locksystem",1,false)) then
            local finished = exports["dg-taskbar"]:taskBar(10000,"Opening bank box.",false,false,playerVeh)
            if (finished == 100) then
                remove = true
    ...
    if (itemid == "Securebriefcase") then
        if (hasEnoughOfItem("Bankboxkey",1,false)) then
            local finished = exports["dg-taskbar"]:taskBar(5000,"Opening briefcase.",false,false,playerVeh)
            if (finished == 100) then
                remove = true
    ...
    if (itemid == "Largesupplycrate") then
        if (hasEnoughOfItem("2227010557",1,false)) then
            local finished = exports["dg-taskbar"]:taskBar(15000,"Opening supply crate.",false,false,playerVeh)
            if (finished == 100) then
                remove = true
            ...
            Citizen.CreateThread(function()
                while finished ~= 100 and not cancelNos do
                    ...
                    if GetEntitySpeed(GetVehiclePedIsIn(player, false)) > 11 then
                        exports["dg-taskbar"]:closeGuiFail()
                        cancelNos = true
                    end
                end
            end)
            finished = exports["dg-taskbar"]:taskBar(20000,"Nitrous")
    ...
    if (itemid == "ciggy") then
        local finished = exports["dg-taskbar"]:taskBar(1000,"Lighting Up",false,false,playerVeh)
        if (finished == 100) then
            Wait(300)
    ...
    if (itemid == "oxygentank") then
        local finished = exports["dg-taskbar"]:taskBar(30000,"Oxygen Tank",true,false,playerVeh)
        if (finished == 100) then        
    ...
    if (itemid == "bandage") then
        TaskItem("amb@world_human_clipboard@male@idle_a", "idle_c", 49,10000,"Healing","dg-hospital:items:bandage",true,itemid,playerVeh)
    end
    ...
    if (itemid == "glucose") then 
        CreateCraftOption("1gcocaine", 1, true)
    ...
    if (itemid == "1gcocaine") then
        local playerPed = GetPlayerPed(-1)
        local finished = exports["dg-taskbar"]:taskBar(3000,"Taking coke",false,false,playerVeh)
        if (finished == 100) then
            Wait(200)
            ...
            -- Citizen.Wait(3000)
            TaskItem("anim@amb@nightclub@peds@", "missfbi3_party_snort_coke_b_male3", 49, 5000, "Coke Gaming", true,itemid,playerVeh)
    ...
    TaskItem("switch@trevor@trev_smoking_meth", "trev_smoking_meth_loop", 49, 5000, "Taking Meth", true,itemid,playerVeh)
    TaskStartScenarioInPlace(playerPed, "WORLD_HUMAN_SMOKING_POT", 0, 1)
    Citizen.Wait(3000)
    ClearPedTasksImmediately(playerPed)
    SetPedMotionBlur(playerPed, true)
    ...
    --SetPedArmour(playerPed, armor+20)
...
--  after wait stop all effects
    ...
    SetTimecycleModifierStrength(0.0)
    local armor = GetPedArmour(GetPlayerPed(-1))
    SetPedArmour(GetPlayerPed(-1), armor+20)
    print(GetPedArmour(playerPed))
    --TriggerEvent('dg-hospital:client:RemoveBleed')
        --TaskItem("switch@trevor@trev_smoking_meth", "trev_smoking_meth_loop", 49, 5000, "Taking Meth", "hadcrack", true,itemid,playerVeh)
    end
    ...
    if (itemid == "cbballs" or itemid == "cbbucket" or itemid == "cbclucker" or itemid == "cbfarmers" or itemid == "cbfowl" or itemid == "cbfries" or itemid == "cbrings" or itemid == "cbrownie" or itemid == "cbveggy") then
        AttachPropAndPlayAnimation("mp_player_inteat@burger", "mp_player_int_eat_burger", 49,6000,"Eating","changehunger",true,itemid,playerVeh)
    end

    if (itemid == "cola" or itemid == "water" or itemid == "cbdrink") then
        ...
        AttachPropAndPlayAnimation("amb@world_human_drinking@beer@female@idle_a", "idle_e", 49,6000,"Drink",true,itemid,playerVeh)
        TriggerEvent('dg_status:add', 'thirst', 400000)
    end
    ...
    --     local finished = exports["dg-taskbarskill"]:taskBar(2500,10)
    --     if (finished == 100) then  
    --         TriggerEvent("attachItemObjectnoanim","crackpipe01")
    --         TriggerEvent("Evidence:StateSet",2,1200)
    --         TriggerEvent("Evidence:StateSet",6,1200)
    ...
    if (itemid == "shitlockpick") then
        ...
        local finished = exports["dg-taskbarskill"]:taskBar(2500,math.random(5,20))
        if (finished == 100) then    
...
AddEventHandler('inv:lockPick', function(isForced,inventoryName,slot)
    ...
    if targetVehicle == 0 then
        ...
        if driverPed ~= 0 then
            ...
            local finished = exports["dg-taskbarskill"]:taskBar(25000,3)
            ...
            local finished = exports["dg-taskbarskill"]:taskBar(2200,10)
        ...
        if targetVehicle ~= 0 and not isForced then
            ...
            local finished = exports["dg-taskbarskill"]:taskBar(math.random(5000,25000),math.random(10,20))
            if finished ~= 100 then
                 lockpicking = false
                return
            end
            ...
            TriggerEvent("civilian:alertPolice",20.0,"lockpick",targetVehicle)
            local finished = exports["dg-taskbarskill"]:taskBar(1500,math.random(5,15))
            if finished ~= 100 then
...
AddEventHandler('veh:repairing', function(inventoryName,slot,itemid)
    ...
    if targetVehicle ~= 0 then
        ...
        if dist < 1.5 then
           ...
           -- print(targetVehicle)
            local finished = exports["dg-taskbarskill"]:taskBar(15000,math.random(10,20))
            SetVehicleFixed(targetVehicle)
            ...
            if finished == 100 then
                ...
                if myJob == "mechanic" then
                    ...
                    if advanced then
                        ...
                        local finished = exports["dg-taskbarskill"]:taskBar(timer,math.random(5,15))
                        if finished ~= 100 then
                            fixingvehicle = false
                            reapiring = false
                            ClearPedTasks(playerped)
...
RegisterCommand("search", function(source, args)
    ...
    if DGCore.GetPlayerData().job.name == 'police' then
...
RegisterCommand("steal", function(source, args)
    ...
    if closestPlayer ~= -1 and closestDistance <= 1.0 then
        ...
        if IsEntityPlayingAnim(searchPlayerPed, 'missfbi5ig_22', 'hands_up_anxious_scientist', 3) or (IsEntityPlayingAnim(searchPlayerPed, "dead", "dead_a", 3)) then
            ...
            local finished = exports["dg-taskbar"]:taskBar(3500, "Robbing")
            TriggerServerEvent("people-search", GetPlayerServerId(closestPlayer)) --COPY THIS
...
RegisterNetEvent('dg-inventory:goldtrade')
AddEventHandler('dg-inventory:goldtrade', function()
	if exports["dg-inventory"]:hasEnoughOfItem("goldbar",70,true) then
		...
		if GoldBars < 1.5 then
			if isTime() then
				...
				if IsControlJustReleased(0,38) then
					if exports["dg-inventory"]:hasEnoughOfItem("goldbar",70,true) then
						local finished = exports["dg-taskbar"]:taskBar(6000,"Trading Bars")
						if finished == 100 then
                            TriggerEvent('dg-inventory:goldtrade')
						end
					end
					Citizen.Wait(1000)
				end
