...
function ShowNoPhoneWarning () 
  if (DGCore == nil) then return end
  DGCore.ShowNotification("You don\'t have a phone ")
end 
      ...
      if IsControlJustPressed(1, KeyOpenClose) then
        if not (IsEntityPlayingAnim(PlayerPedId(), "dead", "dead_a", 3)) then
          hasPhone(function (hasPhone)
            if exports['dg-inventory']:hasEnoughOfItem('mobilephone', 1) then
              TooglePhone()
            else
              ShowNoPhoneWarning()
            end
...
AddEventHandler("gcPhone:receiveMessage", function(message)
  ...
  if message.owner == 0 then
    ...
    if ShowNumberNotification == true then
      text = 'New message from ~y~'.. message.transmitter
      for _,contact in pairs(contacts) do
        if contact.number == message.transmitter then
          text = 'New message from ~g~'.. contact.display
          break
        end
      end
    end

    exports['mythic_notify']:SendAlert('inform', text, 5000, { ['background-color'] = '#A52A2A', ['color'] = '#FFF' })
...
AddEventHandler("gcPhone:acceptCall", function(infoCall, initiator)
 ...
 --   NetworkSetVoiceChannel(infoCall.id + 1)
  --  NetworkSetTalkerProximity(0.0)
	exports.dg_voip:addPlayerToRadio(infoCall.id + 999)
	TokoVoipID = infoCall.id + 999
  end
  if menuIsOpen == false then 
    TooglePhone()
  end
...
AddEventHandler("gcPhone:rejectCall", function(infoCall)
  ...
  --  NetworkSetTalkerProximity(2.5)
	exports.dg_voip:removePlayerFromRadio(TokoVoipID)
	TokoVoipID = nil
  end
  PhonePlayText()
