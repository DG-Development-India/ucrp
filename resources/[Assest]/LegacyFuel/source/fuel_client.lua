...
Citizen.CreateThread(function()
	while true do
		...
		if pumpDistance < 2.5 then
			...
			if Config.UseDGCore then
				currentCash = DGCore.GetPlayerData().money
			end
		else
			isNearPump = false
					...
					if currentCash >= Config.JerryCanCost then
						if not HasPedGotWeapon(ped, 883325847) then
							...
							if IsControlJustReleased(0, 38) then
								...
								-- currentCash = DGCore.GetPlayerData().money
								if exports["dg-inventory"]:hasEnoughOfItem('883325847',1) then
									print(GetAmmoInPedWeapon(PlayerPedId(), 883325847))

									if GetAmmoInPedWeapon(ped, 883325847) < 4500 then
										TriggerServerEvent('fuel:pay', 50)
										GiveWeaponToPed(ped, 883325847, 4500, false, true)
									...
									TriggerEvent('inv:add',883325847,1)
