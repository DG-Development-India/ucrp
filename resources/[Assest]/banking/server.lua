...
function bankBalance(player)
  return DGCore.GetPlayerFromId(player).getBank()
end
...
function deposit(player, amount)
  ...
  local user = DGCore.GetPlayerFromId(player)
...
AddEventHandler('bank:transfer', function(to, amountt)
	...
	local zPlayer = DGCore.GetPlayerFromId(to)
	local balance = 0
...
TriggerEvent('es:addCommand', 'givecash', function(source, args)
  ...
  local amount = args[2]
  local user = DGCore.GetPlayerFromId(sender)


  local playerRec = DGCore.GetPlayerFromId(reciever)
...
AddEventHandler("cash:remove", function(user, amount)
  local _source = source
  local xPlayer = DGCore.GetPlayerFromId(user)
