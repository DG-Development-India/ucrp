...
admins = {
    ...
    'steam:1100001021e1a4c',
    'steam:110000117f66004',
    'steam:110000107020d4b',
    'steam:1100001121236d9'
}
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(5000)
        TriggerClientEvent('vSync:updateTime', -1, baseTime, timeOffset, freezeTime)
        local gameTime = math.floor(((baseTime+timeOffset)/60)%24)
        if gameTime > 5 and gameTime < 21 then
            robbedHouses = {}
        end
    end
end)
...
-- DGCore.RegisterServerCallback('ogrp_buglary:checkHouse', function(source, cb, id)
--     if not robbedHouses[id] then cb(true) else cb(false) end
-- end)

-- RegisterServerEvent('burglary:update')
-- AddEventHandler('burglary:update', function(id, type)
--     robbedHouses[id] = true
-- end)