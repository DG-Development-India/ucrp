...
if Config.MaxInService ~= -1 then
	TriggerEvent('dg-service:activateService', 'police', Config.MaxInService)
end
...
AddEventHandler('playerDropped', function()
	...
	if _source ~= nil then
		...
		local user = DGCore.GetPlayerFromId(_source)
		--local player = user.identifier
		-- Is it worth telling all clients to refresh?
		if xPlayer ~= nil and xPlayer.job ~= nil and xPlayer.job.name == 'police' then
			Citizen.Wait(5000)
			TriggerClientEvent('dg-policejob:updateBlip', -1)
...
RegisterServerEvent('dg-policejob:spawned')
AddEventHandler('dg-policejob:spawned', function()
	local _source = source
	local xPlayer = DGCore.GetPlayerFromId(_source)
...
RegisterServerEvent('dg-policejob:forceBlip')
AddEventHandler('dg-policejob:forceBlip', function()
	TriggerClientEvent('dg-policejob:updateBlip', -1)
end)
...
AddEventHandler('policejob:payBill', function(args)
	local src = source
	local xPlayer = DGCore.GetPlayerFromId(args[1])
...
AddEventHandler('reviveGranted', function(target)
	local xPlayer = DGCore.GetPlayerFromId(source)
		...
		--TriggerClientEvent('DGCore_ambulancejob:revive', target)
		Wait(5000)
...
-- 		TriggerEvent('dg-license:checkLicense', v, 'weapon', function(cb)
-- 		  if cb == false then 
-- 			TriggerEvent('dg-license:addLicense', v, 'weapon', function()
-- 			end)
...
--   RegisterCommand('removelicense', function(source, cb)
-- 	local src = source
-- 	local myPed = GetPlayerPed(src)
-- 	local myPos = GetEntityCoords(myPed)
-- 	local players = DGCore.GetPlayers()
...
-- 		TriggerEvent('dg-license:checkLicense', v, 'weapon', function(cb)
-- 		  if cb == true then 
-- 			TriggerEvent('dg-license:removeLicense', v, 'weapon', function()
-- 			end)
-- 			TriggerClientEvent('DoLongHudText', source, 'You revoked a gun license of [' .. v .. ']' , 1) -- price also changes here
-- 			TriggerClientEvent('DoLongHudText', v, 'Your license have been revoked', 1) -- price also changes here
-- 			TriggerClientEvent('dg-fines:Anim', source)
-- 		  else
-- 			TriggerClientEvent('DoLongHudText', source, 'Failed. ID [ ' .. v .. ' ] Technical Fault, Talk with DOJ' , 2) -- price also changes here
-- 		  end
-- 		end)
-- 	  end
-- 		end
...
--   end)
  ...
  ---cuff stuffs soft cuff---

  DGCore.RegisterServerCallback("dg-policejob:isCuffed",function(source,cb,target)
    cb(cuffed[target]~=nil and cuffed[target])
  end)

RegisterServerEvent("dg-policejob:handcuff")
AddEventHandler("dg-policejob:handcuff",function(player,state,hardcuff,playerheading, playerCoords,  playerlocation)
	print(hardcuff)
    local _source = source
    local xPlayer = DGCore.GetPlayerFromId(_source)
    cuffed[player]=state
	TriggerClientEvent('dg-policejob:handcuff', player , hardcuff, playerheading, playerCoords,  playerlocation)
end)

--search person--
RegisterServerEvent('dg-policejob:message')
AddEventHandler('dg-policejob:message', function(target, msg)
	TriggerClientEvent('dg:showNotification', target, msg)
end)
...
AddEventHandler('sendChatMessage', function(message)
	...
	for i=1, #xPlayers, 1 do
		...
		if xPlayer.job ~= nil and xPlayer.job.name == 'police' then
			...
			TriggerClientEvent('chatMessagess', xPlayers[i], '', 1, '^1Dispatch : ' ..message)
        end
    end
end)