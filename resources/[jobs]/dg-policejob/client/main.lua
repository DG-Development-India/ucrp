...
AddEventHandler('dg:setJob', function(job)
	...
	TriggerServerEvent('dg-policejob:forceBlip')
end)
...
AddEventHandler('police:gsr', function(t)
	...
	local finished = exports["dg-taskbar"]:taskBar(15000,"GSR Testing")
    if finished == 100 then
		t, distance = GetClosestPlayer()
		if(distance ~= -1 and distance < 7) then
			TriggerServerEvent("dg-policejob:checkgsr", GetPlayerServerId(t))
		end
	end
end)
...
AddEventHandler('police:getArrested2', function(cuffer)
	...
	local finished = 0
	if not exports["dg-ambulancejob"]:GetDeath() then
		finished = exports["dg-taskbarskill"]:taskBar(1200,7)
	end
...
AddEventHandler('onResourceStop', function(resource)
	if resource == GetCurrentResourceName() then
		...
		if Config.MaxInService ~= -1 then
			TriggerServerEvent('dg-service:disableService', 'police')
		end
	end
end)
...
Citizen.CreateThread(function() 
	...
	while true do
	  ...
	  if handCuffedWalking or handCuffed then
		  ...
		  DisableControlAction(1, 37, true) --Disables INPUT_SELECT_WEAPON (tab) Actions
		  DisablePlayerFiring(PlayerPedId(), true) -- Disable weapon firing
		  local dead = exports["dg-ambulancejob"]:GetDeath()
...
AddEventHandler( 'Police:Radio', function()
	if isCop then
	    ...
	    if ( DoesEntityExist( ped ) and not IsEntityDead( ped )) and exports["dg-ambulancejob"]:GetDeath() ~= 1 then
	...
	if t ~= nil and t ~= -1 then
		if(distance ~= -1 and distance < 5) then
			...
			if myjob ~= "ambulance" then
				local bandages = exports["dg-inventory"]:getQuantity("bandage")
				if bandages == 0 then
					return
				else
...
AddEventHandler("police:rob", function()
	if not exports["dg-ambulancejob"]:GetDeath() then
		...
		local finished = exports["dg-taskbar"]:taskBar(3500,"Robbing",false,true)	
...
AddEventHandler( 'KneelHU', function()
	...
	if ( DoesEntityExist( player ) and not IsEntityDead( player )) then 
		...
		local finished = exports["dg-taskbar"]:taskBar(2500,"Surrendering")					
...
AddEventHandler('FlipVehicle', function()
	local finished = exports["dg-taskbar"]:taskBar(5000,"Flipping Vehicle Over",false,true)	
...
function deleteVeh(ent)
	...
	local finished = exports["dg-taskbar"]:taskBar(1000,"Impounding",false,true)	
...
AddEventHandler('police:checkInventory', function(isFrisk)
		...
		t, distance, closestPed = GetClosestPlayer()
		if(distance ~= -1 and distance < 5) then
			TriggerServerEvent('dg-policejob:message', GetPlayerServerId(closestPed), _U('being_searched'))
...
RegisterCommand('steal', function(source, args, rawCommand)
	local cp, cd = DGCore.Game.GetClosestPlayer()
	if cp ~= -1 and cd <= 2.0 then

		local closestPed = GetPlayerPed(cp)
		if (IsEntityPlayingAnim(closestPed, "dead", "dead_a", 3)) then
			...
			TriggerServerEvent('dg-policejob:message', GetPlayerServerId(cp), _U('being_looted'))
			TriggerServerEvent("people-search", GetPlayerServerId(cp))
		else

			DGCore.TriggerServerCallback("dg-policejob:isCuffed",function(cuffed)
				if not cuffed then
					exports['mythic_notify']:SendAlert('inform', 'You can not steal from this person', 2500)
...
RegisterNetEvent('dg-policejob:updateBlip')
AddEventHandler('dg-policejob:updateBlip', function()
...
AddEventHandler('playerSpawned', function(spawn)
	if not hasAlreadyJoined then
		TriggerServerEvent('dg-policejob:spawned')
	end
...
AddEventHandler("tp:pdrevive", function()
    ...
    if closestPlayer ~= -1 and closestDistance <= 3.0 then
		...
		TriggerServerEvent('dg-ambulancejob:revivePD', GetPlayerServerId(closestPlayer))
...
RegisterNetEvent("dg-policejob:checkCuff")
AddEventHandler("dg-policejob:checkCuff", function(hardcuff,cf)
	...
	if distance~=-1 and distance<=3.0 then
        DGCore.TriggerServerCallback("dg-policejob:isCuffed",function(cuffed)
			if not cuffed then
				...
				TriggerServerEvent("dg-policejob:handcuff",target_id,true,hardcuff,playerheading,playerCoords,playerlocation)
			else
				if not cf then
					TaskPlayAnim(GetPlayerPed(-1), 'mp_arresting', 'a_uncuff', 8.0, -8, 5000, 0, 0, 0, 0, 0)
					TriggerServerEvent("dg-policejob:handcuff",target_id,false,hardcuff,playerheading,playerCoords,playerlocation)
				else
					--exports['mythic_notify']:SendAlert('inform', 'The player is already cuffed', 2500, {})
					TriggerEvent('DoLongHudText', 'The player is already cuffed!', 1)
				end
            end
...
local hardcuff
RegisterNetEvent('dg-policejob:handcuff')
AddEventHandler('dg-policejob:handcuff', function(cufftype,playerheading,playercoords,playerlocation)
	local playerPed = GetPlayerPed(-1)
...
AddEventHandler('police:panic', function(source, args, rawCommand)
...
-- 		--local job = DGCore.GetPlayerData().job.name
-- 		if distance < 10 then
-- 			if PlayerData.job and PlayerData.job.name == "police" then
-- 			--if job == "police" then
-- 				if distance < 1 then
-- 					DrawText3Ds(x,y,z, 'Press [~g~E~s~] to open Armory')
...
-- 			--if job == "police" then
-- 				if distance < 1 then
-- 					DrawText3Ds(x,y,z, 'Press [~g~E~s~] to open high command Armory')
-- 					if IsControlJustReleased(0,  46) then
-- 						if exports['dg-inventory']:hasEnoughOfItem('securitygold', 1)then
-- 						TriggerEvent("server-inventory-open", "113", "Shop");
-- 						else
-- 							exports['mythic_notify']:DoLongHudText('error', 'You are not authorized !')
-- 						end
-- 					end
-- 				end
...
-- 			--if job == "police" then
-- 				if distance < 1 then
-- 					DrawText3Ds(x,y,z, 'Press [~g~E~s~] to take attachments')
-- 					if IsControlJustReleased(0,  46) then
-- 						TriggerEvent("server-inventory-open", "117", "Shop");
-- 					end
-- 				end
...
-- 				if distance < 1 then
-- 					DrawText3Ds(x,y,z, 'Press [~g~E~s~] to make coffee')
-- 					if IsControlJustReleased(0,  46) then
-- 						playAnim("mini@repair", "fixing_a_player", 10000)
-- 						local finished = exports["dg-taskbar"]:taskBar(10000,"cooking")
--       					if (finished == 100) then
...
-- 			--if job == "beanmachine" then
-- 				if distance < 1 then
-- 					DrawText3Ds(x,y,z, 'Press [~g~E~s~] to get icecube')
-- 					if IsControlJustReleased(0,  46) then
-- 						playAnim("mini@repair", "fixing_a_player", 10000)
-- 						local finished = exports["dg-taskbar"]:taskBar(10000,"taking icecube")
--       					if (finished == 100) then
-- 							pos = GetEntityCoords(PlayerPedId(), false)
-- 							TriggerEvent('player:receiveItem', "icecube", 1)
-- 						end
-- 					end
-- 				end
...
-- -- 		local distance = GetDistanceBetweenCoords(coords, x,y,z, true)
-- -- 		if distance < 10 then
-- -- 			if PlayerData.job ~= nil and PlayerData.job.name == "cbells" then
-- -- 				if distance < 1 then
-- -- 					DrawText3Ds(x,y,z, 'Press [~g~E~s~] to cut chicken')
-- -- 					if IsControlJustReleased(0,  46) then
-- -- 						if exports['dg-inventory']:hasEnoughOfItem('packaged_chicken', 1)then
-- -- 							playAnim("anim@amb@business@coc@coc_unpack_cut_left@", "coke_cut_v1_coccutter", 10000)
-- -- 							local finished = exports["dg-taskbar"]:taskBar(10000,"cutting")
-- --       						if (finished == 100) then
-- -- 							pos = GetEntityCoords(PlayerPedId(), false)
-- -- 							cuttingchicken()
-- -- 							end
-- -- 						else
-- -- 							exports['mythic_notify']:DoLongHudText('error', 'Really, do you have enough packed chicken?')
-- -- 						end
-- -- 					end
-- -- 				end
...
-- -- function for cutting chicken
-- function cuttingchicken()
-- 	isMenuOpen = true
	
		
-- 		local elements = {
-- 			{label = 'Small pieces',   value = 'spieces'},
-- 			{label = 'Medium pieces',      value = 'mpieces'},
-- 			{label = 'Large pieces',       value = 'lpieces'},
-- 		}

-- 		DGCore.UI.Menu.CloseAll()
    
-- 		DGCore.UI.Menu.Open('default', GetCurrentResourceName(), 'jeweler_actions', {
-- 			align    = 'top-right',
-- 			elements = elements
-- 		}, function(data, menu)
-- 			if data.current.value == 'spieces' then
-- 				menu.close()
-- 				if exports['dg-inventory']:hasEnoughOfItem('packaged_chicken', 1)then
-- 					TriggerEvent('inventory:removeItem', "packaged_chicken", 1)
-- 					TriggerEvent('player:receiveItem', "smallchicken", 10)
-- 				end
-- 			elseif data.current.value == 'mpieces' then
-- 				menu.close()
-- 				if exports['dg-inventory']:hasEnoughOfItem('packaged_chicken', 1)then
-- 					TriggerEvent('inventory:removeItem', "packaged_chicken", 1)
-- 					TriggerEvent('player:receiveItem', "mediumchicken", 5)
-- 				end
-- 			elseif data.current.value == 'lpieces' then
-- 				menu.close()
-- 				if exports['dg-inventory']:hasEnoughOfItem('packaged_chicken', 1)then
-- 					TriggerEvent('inventory:removeItem', "packaged_chicken", 1)
-- 					TriggerEvent('player:receiveItem', "largechicken", 3)
-- 				end
-- 			end
...
-- 			--if job == "cbells" then
-- 				if distance < 1 then
-- 					DrawText3Ds(x,y,z, 'Press [~g~E~s~] to fry chicken')
-- 					if IsControlJustReleased(0,  46) then
-- 						if exports['dg-inventory']:hasEnoughOfItem('mediumchicken', 1)then
-- 							playAnim("mini@repair", "fixing_a_player", 10000)
-- 							local finished = exports["dg-taskbar"]:taskBar(10000,"Frying chicken")
--       						if (finished == 100) then
-- 							pos = GetEntityCoords(PlayerPedId(), false)
-- 							friedchicken1()
-- 							end
-- 						else
-- 							exports['mythic_notify']:DoLongHudText('error', 'You do not have enough chicken to fry')
-- 						end
-- 					end
-- 				end
...
-- -- function for cutting chicken
-- function friedchicken1()
-- 	isMenuOpen = true
	
		
-- 		local elements = {
-- 			{label = 'Fried Chicken(Medium)',   value = 'fchickenm'},
-- 			{label = 'Fried Chicken(Large)',      value = 'fchickenl'},
-- 		}
...
-- 			elements = elements
-- 		}, function(data, menu)
-- 			if data.current.value == 'fchickenm' then
-- 				menu.close()
-- 				if exports['dg-inventory']:hasEnoughOfItem('mediumchicken', 1)then
-- 					TriggerEvent('inventory:removeItem', "mediumchicken", 1)
-- 					TriggerEvent('player:receiveItem', "friedchickenm", 1)
-- 				end
-- 			elseif data.current.value == 'fchickenl' then
-- 				menu.close()
-- 				if exports['dg-inventory']:hasEnoughOfItem('largechicken', 1)then
-- 					TriggerEvent('inventory:removeItem', "largechicken", 1)
-- 					TriggerEvent('player:receiveItem', "friedchickenl", 1)
-- 				end
-- 			end
-- 		end, function(data,menu)
-- 			menu.close()
...
-- 								  if (finished == 100) then
-- 								pos = GetEntityCoords(PlayerPedId(), false)
-- 								TriggerEvent("server-inventory-open", "123", "Craft");
-- 								end
-- 							--else
-- 								--exports['mythic_notify']:DoLongHudText('error', 'You do not have enough chicken to fry')
-- 							--end
...
-- 				--if job == "cbells" then
-- 					if distance < 1 then
-- 						DrawText3Ds(x,y,z, 'Press [~g~E~s~] to packing food')
-- 						if IsControlJustReleased(0,  46) then
-- 							if exports['dg-inventory']:hasEnoughOfItem('bucket', 1)then
-- 								playAnim("mini@repair", "fixing_a_player", 10000)
-- 								local finished = exports["dg-taskbar"]:taskBar(10000,"making food")
-- 								  if (finished == 100) then
-- 								pos = GetEntityCoords(PlayerPedId(), false)
...
-- 				--if job == "cbells" then
-- 					if distance < 1 then
-- 						DrawText3Ds(x,y,z, 'Press [~g~E~s~] to make drink')
-- 						if IsControlJustReleased(0,  46) then
-- 							if exports['dg-inventory']:hasEnoughOfItem('bucket', 1)then
-- 								playAnim("mini@repair", "fixing_a_player", 10000)
-- 								local finished = exports["dg-taskbar"]:taskBar(10000,"making food")
-- 								  if (finished == 100) then
-- 								pos = GetEntityCoords(PlayerPedId(), false)
-- 								TriggerEvent("server-inventory-open", "124", "Craft");
-- 								end
-- 							else
-- 								exports['mythic_notify']:DoLongHudText('error', 'You do not have enough stuff')
-- 							end
...
-- 			-- cbells drink crafting 2 (soft drinks)
-- Citizen.CreateThread(function()
...
-- 							-- if exports['dg-inventory']:hasEnoughOfItem('bucket', 1)then
-- 								playAnim("mini@repair", "fixing_a_player", 10000)
-- 								local finished = exports["dg-taskbar"]:taskBar(10000,"making drink")
-- 								  if (finished == 100) then
-- 								pos = GetEntityCoords(PlayerPedId(), false)
-- 								TriggerEvent("server-inventory-open", "125", "Shop");
-- 								end
-- 							-- else
-- 							-- 	exports['mythic_notify']:DoLongHudText('error', 'You do not have enough stuff')
-- 							-- end
