...
function InitiateMenus(isMotorcycle, vehicleHealth)
    ...
    if vehicleHealth < 1000.0 then
        ...
        createMenu("repairMenu", "Welcome to the tunershop", "Repair Vehicle")
        populateMenu("repairMenu", -1, "Repair", "")
        finishPopulatingMenu("repairMenu")
    end

    --#[Main Menu]#--
    createMenu("mainMenu", "Welcome to the tunershop", "Choose a Category")
        ...
        if currentMenuItem2 ~= "Installed" then
            if isMenuActive("modMenu") then
                if currentCategory == 18 then --Turbo
                    ...
                    if exports['dg-inventory']:hasEnoughOfItem('turbo', 1) then
                        if AttemptPurchase("turbo") then
                            ...
                            TriggerEvent("inventory:removeItem",'turbo', 1)
                        else
                            updateMenuStatus("Not Enough Money!")
                        end
                    else
                        updateMenuStatus("You don\'t have a turbo")
                    end

                elseif currentCategory == 11  then
                    
                    if exports['dg-inventory']:hasEnoughOfItem('enginekit', 1) then
                        ...
                        updateMenuStatus("You don\'t have a engine upgrade kit")
                    end
                    
                elseif currentCategory == 12 then

                    if exports['dg-inventory']:hasEnoughOfItem('brakekit', 1) then
                        
                        if AttemptPurchase("performance", currentMenuItemID) then
                            ApplyMod(currentCategory, currentMenuItemID)
                            playSoundEffect("wrench", 0.4)
                            updateItem2Text(currentMenu, currentMenuItemID, "Installed")
                            TriggerEvent("inventory:removeItem",'brakekit', 1)
                            updateMenuStatus("Purchased")
                        else
                            updateMenuStatus("Not Enough Money")
                        end
                        ...
                        updateMenuStatus("You don\'t have a brake upgrade kit")
                    end
                    
                elseif currentCategory== 13 then

                    if exports['dg-inventory']:hasEnoughOfItem('transkit', 1) then
                        if AttemptPurchase("performance", currentMenuItemID) then
                            ApplyMod(currentCategory, currentMenuItemID)
                            playSoundEffect("wrench", 0.4)
                            updateItem2Text(currentMenu, currentMenuItemID, "Installed")
                            TriggerEvent("inventory:removeItem",'transkit', 1)
                            updateMenuStatus("Purchased")
                        else
                            updateMenuStatus("Not Enough Money")
                        end
                    else
                        updateMenuStatus("You don\'t have a transmission upgrade kit")
                    end
                ...
                elseif currentCategory == 0 then 
                    
                    if exports['dg-inventory']:hasEnoughOfItem('spoiler', 1) then
                        if AttemptPurchase("cosmetics") then
                            ApplyMod(currentCategory, currentMenuItemID)
                            playSoundEffect("wrench", 0.4)
                            updateItem2Text(currentMenu, currentMenuItemID, "Installed")
                            TriggerEvent("inventory:removeItem",'spoiler', 1)
                            updateMenuStatus("Purchased")
                        else
                            updateMenuStatus("Not Enough Money")
                        end
                    else
                        updateMenuStatus("You don't have a spoiler")
                    end

                elseif currentCategory == 1 then 
                    if exports['dg-inventory']:hasEnoughOfItem('fbumper', 1) then
                        if AttemptPurchase("cosmetics") then
                            ApplyMod(currentCategory, currentMenuItemID)
                            playSoundEffect("wrench", 0.4)
                            updateItem2Text(currentMenu, currentMenuItemID, "Installed")
                            TriggerEvent("inventory:removeItem",'fbumper', 1)
                            updateMenuStatus("Purchased")
                        else
                            updateMenuStatus("Not Enough Money")
                        end
                    else
                        updateMenuStatus("You don't have a front bumper")
                    end

                elseif currentCategory == 2 then 
                    if exports['dg-inventory']:hasEnoughOfItem('rbumper', 1) then
                        if AttemptPurchase("cosmetics") then
                            ApplyMod(currentCategory, currentMenuItemID)
                            playSoundEffect("wrench", 0.4)
                            updateItem2Text(currentMenu, currentMenuItemID, "Installed")
                            TriggerEvent("inventory:removeItem",'rbumper', 1)
                            updateMenuStatus("Purchased")
                        else
                            updateMenuStatus("Not Enough Money")
                        end
                    else
                        updateMenuStatus("You don't have a rear bumper")
                    end
                
                elseif currentCategory == 3 then 
                    if exports['dg-inventory']:hasEnoughOfItem('sskirts', 1) then
                        if AttemptPurchase("cosmetics") then
                            ApplyMod(currentCategory, currentMenuItemID)
                            playSoundEffect("wrench", 0.4)
                            updateItem2Text(currentMenu, currentMenuItemID, "Installed")
                            TriggerEvent("inventory:removeItem",'sskrits', 1)
                            updateMenuStatus("Purchased")
                        else
                            updateMenuStatus("Not Enough Money")
                        end
                    else
                        updateMenuStatus("You don't have side skirts")
                    end
                
                elseif currentCategory == 4 then 
                    if exports['dg-inventory']:hasEnoughOfItem('exhaustpipe', 1) then
                        if AttemptPurchase("cosmetics") then
                            ApplyMod(currentCategory, currentMenuItemID)
                            playSoundEffect("wrench", 0.4)
                            updateItem2Text(currentMenu, currentMenuItemID, "Installed")
                            TriggerEvent("inventory:removeItem",'exhaustpipe', 1)
                            updateMenuStatus("Purchased")
                        else
                            updateMenuStatus("Not Enough Money")
                        end
                    else
                        updateMenuStatus("You don't have an exhaust pipe")
                    end
                
                elseif currentCategory == 7 then 
                    if exports['dg-inventory']:hasEnoughOfItem('carhood', 1) then
                        if AttemptPurchase("cosmetics") then
                            ApplyMod(currentCategory, currentMenuItemID)
                            playSoundEffect("wrench", 0.4)
                            updateItem2Text(currentMenu, currentMenuItemID, "Installed")
                        ...
                        updateMenuStatus("You don't have a car hood")
                    end
                
                elseif currentCategory == 10 then 
                    if exports['dg-inventory']:hasEnoughOfItem('carbonfibre', 10) then
                        if AttemptPurchase("cosmetics") then
                            ApplyMod(currentCategory, currentMenuItemID)
                            playSoundEffect("wrench", 0.4)
                            updateItem2Text(currentMenu, currentMenuItemID, "Installed")
                            TriggerEvent("inventory:removeItem",'carbonfibre', 10)
                            updateMenuStatus("Purchased")
                        else
                            updateMenuStatus("Not Enough Money")
                        end
                    else
                        updateMenuStatus("You need 10 carbon fibre sheets")
                    end

                elseif currentCategory == 14 then 
                    if exports['dg-inventory']:hasEnoughOfItem('sound', 1) then
                        if AttemptPurchase("cosmetics") then
                            ApplyMod(currentCategory, currentMenuItemID)
                            playSoundEffect("wrench", 0.4)
                            updateItem2Text(currentMenu, currentMenuItemID, "Installed")
                            TriggerEvent("inventory:removeItem",'sound', 1)
                            updateMenuStatus("Purchased")
                        else
                            updateMenuStatus("Not Enough Money")
                        end
                    else
                        updateMenuStatus("You don't have a sound system")
                    end
            ...
            elseif isMenuActive("ResprayMenu") then
                if exports['dg-inventory']:hasEnoughOfItem('spraykit', 1) then
                    if AttemptPurchase("respray") then
                        ...
                        updateItem2Text(currentMenu, currentMenuItemID, "Installed")
                        TriggerEvent("inventory:removeItem",'spraykit', 1)
                        updateMenuStatus("Purchased")
                    else
                        updateMenuStatus("Not Enough Money")
                    end
                else
                    updateMenuStatus("You don\'t have a spray kit")
                end
                ...
                if currentWheelCategory == 20 then
                    ...
                    if currentWheelCategory == -1 then --Custom Wheels
                        ...
                        if currentWheel == -1 then
                            ...
                            if exports['dg-inventory']:hasEnoughOfItem('tire', 4) then
                                if AttemptPurchase("customwheels") then
                                    ApplyCustomWheel(currentMenuItemID)
                                    playSoundEffect("wrench", 0.4)
                                    updateItem2Text(currentMenu, currentMenuItemID, "Installed")
                                    TriggerEvent("inventory:removeItem",'tire', 4)
                                    updateMenuStatus("Purchased")
                                else
                                    updateMenuStatus("Not Enough Money")
                                end
                            else
                                updateMenuStatus("You need 4 tires to upgrade")
                            end
                        end
                    else
                        local currentWheel = GetCurrentWheel()
                        ...
                        if currentCustomWheelState and currentWheel == -1 then
                            ...
                            if exports['dg-inventory']:hasEnoughOfItem('rims', 4) then
                                if AttemptPurchase("wheels") then
                                    ApplyWheel(currentCategory, currentMenuItemID, currentWheelCategory)
                                    playSoundEffect("wrench", 0.4)
                                    updateItem2Text(currentMenu, currentMenuItemID, "Installed")
                                    TriggerEvent("inventory:removeItem",'rims', 4)
                                    updateMenuStatus("Purchased")
                                else
                                    updateMenuStatus("Not Enough Money")
                                end
                            else
                                updateMenuStatus("You need 4 rims to upgrade")
                            end
                        end
                    end
                end
            ...
            elseif isMenuActive("NeonsSideMenu") then
                ...
                if exports['dg-inventory']:hasEnoughOfItem('neonkit', 1) then
                    if AttemptPurchase("neonside") then
                        ApplyNeon(currentNeonSide, currentMenuItemID)
                        playSoundEffect("wrench", 0.4)
                        updateItem2Text(currentMenu, currentMenuItemID, "Installed")
                        TriggerEvent("inventory:removeItem",'neonkit', 1)
                        updateMenuStatus("Purchased")
                    else
                        updateMenuStatus("Not Enough Money")
                    end
                else
                    updateMenuStatus("You don't have a neon kit")
                end

            else
                if currentMenu == "repairMenu" then
                    if AttemptPurchase("repair") then
                        updateMenuStatus("Get this vehicle repaired by mechanics")
                    else
                        updateMenuStatus("Not Enough Money")
                    end
                ...
                elseif currentMenu == "HeadlightsMenu" then
                    ...
                    if exports['dg-inventory']:hasEnoughOfItem('hlights', 1) then
                        if AttemptPurchase("headlights") then
                            ApplyXenonLights(currentCategory, currentMenuItemID)
                            playSoundEffect("wrench", 0.4)
                            updateItem2Text(currentMenu, currentMenuItemID, "Installed")
                            TriggerEvent("inventory:removeItem",'hlights', 1)
                            updateMenuStatus("Purchased")
                        else
                            updateMenuStatus("Not Enough Money")
                        end
                    else
                        updateMenuStatus("You need a pair of headlights")
                    end
