...
listOn = false
	while true do
		...
		if IsControlJustPressed(1, 38) and IsPedInAnyVehicle(GetPlayerPed(-1), false) ~= 1 then
			...
			-- if GetDistanceBetweenCoords(PlayerPos, -1172.6, -1572.13, 4.67, true) <= 5 then						
			-- 	-- selling bud
			-- 	local smallbud = exports["dg-inventory"]:hasEnoughOfItem("smallbud",5,false)
			-- 	if smallbud then
			...
			-- 		JustDriedWeed = true
			-- 	end
			-- 	local wetbud = exports["dg-inventory"]:hasEnoughOfItem("wetbud",5,false)
			-- 		if wetbud then

			-- 		local finished = exports["dg-taskbar"]:taskBar(10000,"Drying Weed",false,false,playerVeh)
			...
			-- 	TriggerEvent("server-inventory-open", "11", "Craft")
			-- end
