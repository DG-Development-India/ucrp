...
AddEventHandler("dg:playerLoaded", function(playerData)
	DGCore.PlayerData = playerData
end)

RegisterNetEvent("dg:setJob")
AddEventHandler("dg:setJob", function(newJob)
	DGCore.PlayerData["job"] = newJob
end)
						...
						if dstCheck <= markerSize - 0.1 then
							...
							if usable then
								if IsControlJustPressed(0, 38) and Menu.hidden then
									...
									DGCore.TriggerServerCallback("betrayed_garage:obtenerVehiculos", function(fetchedVehicles)
