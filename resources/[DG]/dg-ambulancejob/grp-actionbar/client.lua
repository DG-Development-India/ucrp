	...
	while true do
		...
		if IsPedShooting(ped) then
			...
			if throwableWeapons[weapon] then
				if exports["dg-inventory"]:hasEnoughOfItem(weapon,1,false) then
...
function unholster1h(weaponHash)
	...
	if weaponHash ~= -538741184 and weaponHash ~= 615608432 then
        ...
        DGCore.TriggerServerCallback('dg-inventory:getAmmoCount', function(ammoCount)
...
function updateAmmo()
    ...
    if type(ammoType) == 'number' then 
        ...
        TriggerServerEvent('dg-inventory:updateAmmoCount', hash, ammoCount)
