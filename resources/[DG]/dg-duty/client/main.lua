...
----markers
AddEventHandler('dg-duty:hasEnteredMarker', function (zone)
  ...
  if zone == 'PdmDuty' then
    CurrentAction     = 'pdm_duty'
    CurrentActionMsg  = _U('duty')
    CurrentActionData = {}
  end
end)
  ...
  while true do
    ...
    if CurrentAction ~= nil then
      ...
      if IsControlPressed(0, Keys['E']) then
        if CurrentAction == 'ambulance_duty' then
          ...
          if PlayerData.job.name == 'ambulance' then
            ...
            TriggerServerEvent("TokoVoip:addPlayerToRadio", 2.0, GetPlayerServerId(PlayerId()))
            Wait(1000)
          end
        else
      ...
      if CurrentAction == 'doj_duty' then
        ...
        if CurrentAction == 'pdm_duty' then
          if PlayerData.job.name == 'pdm' or PlayerData.job.name == 'offpdm' then
            TriggerServerEvent('duty:pdm')
  ...
  while true do
    ...
    for k,v in pairs(Config.Zones) do
      if(GetDistanceBetweenCoords(coords, v.Pos.x, v.Pos.y, v.Pos.z, true) < Config.DrawDistance) then
        closestString = "[~g~E~s~] - On/Off Duty"
        DGCore.Game.Utils.DrawText3D(vector3(v.Pos.x, v.Pos.y, v.Pos.z), closestString, 0.4)
      end
    end
  end
end)
...
Citizen.CreateThread(function ()
  while true do
    ...
    if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
      ...
      TriggerEvent('dg-duty:hasEnteredMarker', currentZone)
    end

    if not isInMarker and HasAlreadyEnteredMarker then
      HasAlreadyEnteredMarker = false
      TriggerEvent('dg-duty:hasExitedMarker', LastZone)
    end
  end
end)