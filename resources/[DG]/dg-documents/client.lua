...
Citizen.CreateThread(function()
    ...
    while DGCore.IsPlayerLoaded == false do
        Citizen.Wait(10)
    end
...
function CopyFormToPlayer(aPlayer)
    --TriggerServerEvent('dg_documents:CopyToPlayer', GetPlayerServerId(player), aDocument)
    TriggerServerEvent('dg_documents:CopyToPlayer', aPlayer, CURRENT_DOCUMENT)
...
function DeleteDocument(aDocument)
    ...
    DGCore.TriggerServerCallback('dg_documents:deleteDocument', function (cb)
        if cb == true then
            --remove form_close
...
function CreateNewForm(aDocument)
    ...
    PlayerData = DGCore.GetPlayerData()
    DGCore.TriggerServerCallback('dg_documents:getPlayerDetails', function (cb_player_details)
...
function ShowDocument(aPlayer)
     --   print("ssss: " .. dump(aPlayer))
        TriggerServerEvent('dg_documents:ShowToPlayer', aPlayer, CURRENT_DOCUMENT)
        CURRENT_DOCUMENT = nil
        CloseMenu()
end

RegisterNetEvent('dg_documents:viewDocument')
AddEventHandler('dg_documents:viewDocument', function( data )
...
RegisterNetEvent('dg_documents:copyForm')
AddEventHandler('dg_documents:copyForm', function( data )
...
function GetAllUserForms()
    ...
    DGCore.TriggerServerCallback('dg_documents:getPlayerDocuments', function (cb_forms)
...
RegisterNUICallback('form_submit', function(data, cb)
   -- print("received: " .. dump(data))
    CloseMenu()
    DGCore.TriggerServerCallback('dg_documents:submitDocument', function (cb_form)
        if cb_form ~= nil then
...
function GetNeareastPlayers()
    ...
    local players, nearbyPlayer = DGCore.Game.GetPlayersInArea(GetEntityCoords(playerPed), 3.0)
