...
function init() {
	...
	websocket.onmessage = (evt) => {
		...
		if (evt.data == 'startedtalking') {
			$.post('http://dg_voip/setPlayerTalking', JSON.stringify({state: 1}));
		}
		if (evt.data == 'stoppedtalking') {
			$.post('http://dg_voip/setPlayerTalking', JSON.stringify({state: 0}));
		}
	};
...
function updateTokovoipInfo(msg) {
	...
	switch (voipStatus) {
		...
		case PLUGIN_INITIALIZING:
			...
			break;
		case WRONG_SERVER:
			msg = `Wrong TeamSpeak server!`;
			screenMessage = 'Wrong TeamSpeak server';
			color = 'red';
			break;
		case WRONG_CHANNEL:
			msg = `Wrong TeamSpeak channel, Please join: <font color="#01b0f0">${voip.plugin_data.TSChannelWait && voip.plugin_data.TSChannelWait !== '' && voip.plugin_data.TSChannelWait || voip.plugin_data.TSChannel}</font>`;
	...
	if (msg) {
		document.getElementById('tokovoipInfo').innerHTML = `TokoVoip: <font color = ${color}>${msg}</font>`;
	}
...
function updateScriptData(key, data) {
	...
	$.post(`http://dg_voip/updatePluginData`, JSON.stringify({
		payload: {
			key,
			data,
		}
