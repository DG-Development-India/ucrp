...
function dg.Admin.CheckForSessions(self)
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            local players = dg._Admin.Players

            for k,v in pairs(players) do
                local src = v.src
                local playerId = GetPlayerFromServerId(src)
                if not src then
                    dg._Admin.Players[src].sessioned = true
                else
                    if not NetworkIsPlayerActive(playerId) or not NetworkIsPlayerConnected(playerId) then dg._Admin.Players[src].sessioned = true end
                end
            end
        end
    end)
...
stop = true
function dg.Admin.BeginAFK(self, stop)
    if stop then
        afk.isAfk = false
        afk.msgAFK = false
        afk.stringToType = ""
    ...
    afk.event = AddEventHandler("dg-admin:afkStringCheck", function(text)
        if string.lower(text) == afk.stringToType then dg.Admin:BeginAFK(true) return end
    end)
    ...
    Citizen.CreateThread(function()
        ...
        while true do
            ...
            if curTime - beginTime >= 300000 then TriggerServerEvent("dg-admin:Disconnect", "AFK Kick") return end
        end
    end)
end

function dg.Admin.AFKCheck(self)
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            local rank = dg.Admin:GetPlayerRank()
            local rankData = dg.Admin:GetRankData(rank)
...
RegisterNetEvent("dg-admin:afkStringCheck")

RegisterNetEvent("dg-admin:setStatus")
AddEventHandler("dg-admin:setStatus", function(src, status)
    local player = dg._Admin.Players[src]
    if not player then return else dg._Admin.Players[src].status = status end
end)

RegisterNetEvent("dg-admin:sendPlayerInfo")
AddEventHandler("dg-admin:sendPlayerInfo", function(data, discData)
    dg._Admin.Players = data
    dg._Admin.DiscPlayers = discData
end)

RegisterNetEvent("dg-admin:RemovePlayer")
AddEventHandler("dg-admin:RemovePlayer", function(data)
    dg._Admin.DiscPlayers[data.src] = data
    dg._Admin.Players[data.src] = nil
end)

RegisterNetEvent("dg-admin:RemoveRecent")
AddEventHandler("dg-admin:RemoveRecent", function(data)
    if data ~= nil then
        dg._Admin.DiscPlayers[data.src] = nil
    end
end)

RegisterNetEvent("dg-admin:AddPlayer")
AddEventHandler("dg-admin:AddPlayer", function(player)
    dg._Admin.Players[player.src] = player
end)

RegisterNetEvent('dg-admin:teleportUser')
AddEventHandler('dg-admin:teleportUser', function(x, y, z)
    SetEntityCoords(PlayerPedId(), x, y, z)
    TriggerEvent('DoLongHudText', 'You were summoned')
end)

RegisterNetEvent("dg-admin:AddAllPlayers")
AddEventHandler("dg-admin:AddAllPlayers", function(data)
    dg._Admin.Players[data.src] = data
end)

function dg.Admin.RunCommand(self, args)
    if not args or not args.command then return end
    TriggerServerEvent("dg-admin:runCommand", args)
end

function dg.Admin.RunClCommand(self, cmd, args)
    if not cmd or not self:CommandExists(cmd) then return end
    self:GetCommandData(cmd).runclcommand(args)
end

RegisterNetEvent("dg-admin:RunClCommand")
AddEventHandler("dg-admin:RunClCommand", function(cmd, args)
    dg.Admin:RunClCommand(cmd, args)
end)

RegisterNetEvent("dg-admin:updateData")
AddEventHandler("dg-admin:updateData", function(src, type, data)
    if not src or not type or not data then return end
    if not dg._Admin.Players[src] then return end
    
    dg._Admin.Players[src][type] = data
end)

RegisterNetEvent("dg-admin:noLongerAdmin")
AddEventHandler("dg-admin:noLongerAdmin", function()
    dg._Admin.Players = {}
    
    for k,v in pairs(dg._Admin.Menu.Menus) do
        if WarMenu.IsMenuOpened(k) then WarMenu.CloseMenu() end
    end
end)

RegisterNetEvent("dg-admin:bringPlayer")
AddEventHandler("dg-admin:bringPlayer", function(targPos)
    local ped = PlayerPedId()
...
RegisterNetEvent("dg-admin:bring")
AddEventHandler("dg-admin:bring", function(target)
    ...
    TriggerServerEvent("dg-admin:bringPlayerServer",pos,target)   
end)



 
RegisterNetEvent("dg-admin:runSpawnCommand")
AddEventHandler("dg-admin:runSpawnCommand", function(model, livery)
    Citizen.CreateThread(function()
...
function dg.Admin.teleportMarker(self)
...
function dg.Admin.GetBanTimeFromString(self, time)
...
RegisterNetEvent("dg-admin:ReviveInDistance")
AddEventHandler("dg-admin:ReviveInDistance", function()
    local playerList = {}
