...
dg = dg or {}
dg.Admin = dg.Admin or {}
dg.Admin.DB = {}
dg._Admin = dg._Admin or {}
dg._Admin.Ranks = {}
dg._Admin.Commands = {}
dg._Admin.Players = {}
dg._Admin.DiscPlayers = {}
dg._Admin.CurAdmins = {}

function dg.Admin.Log(self, log, user)
...
function dg.Admin.ChatPrint(self, msg, target)

end

function dg.Admin.RankExists(self, rank)
    rank = tostring(rank)
...
function dg.Admin.CommandExists(self, cmd)
    cmd = tostring(cmd)
...
function dg.Admin.RankInheritsCommand(self, rank, cmd)
...
function dg.Admin.GetPlayerRank(self, user)
    if not IsDuplicityVersion() then
        return group
    end
end

function dg.Admin.RankHasCommand(self, rank, cmd)
...
function dg.Admin.GetRanks(self)
    return dg._Admin.Ranks
end

function dg.Admin.GetRankData(self, rank)
    if not rank then return false end
...
function dg.Admin.GetCommands(self)
    return dg._Admin.Commands
end

function dg.Admin.GetCategories(self)
    local tmp = {}
...
function dg.Admin.GetCommandData(self, cmd)
    if not cmd then return false end
...
function dg.Admin.IsAdmin(self, user)
...
function dg.Admin.IsSuperAdmin(self, user)
    if not IsDuplicityVersion() then
...
function dg.Admin.IsValidUser(self, user)
    if not user then return false end
...
function dg.Admin.CanTarget(self, user, target)
...
function dg.Admin.CanTargetRank(self, user, rank)
...
function dg.Admin.AddCommand(self, cmd)
    local tmp = {}
    ...
    if chicken then
        ...
        tmp.runcommand = function(args) args.command = tmp.command dg.Admin:RunCommand(args) end
...
function dg.Admin.AddRank(self, rank, data)
    local tmp = {}
...
function dg.Admin.SetStatus(self, status, src)
    ...
    if IsDuplicityVersion() then
        ...
        if not player then else dg._Admin.Players[src].status = status end

        for k,v in pairs(dg._Admin.CurAdmins) do
            TriggerClientEvent("dg-admin:setStatus", k, src, status)
        end

        return
    end

    TriggerServerEvent("dg-admin:setStatus", status)
end

AddEventHandler("np-base:exportsReady", function()
    exports["np-base"]:addModule("Admin", dg.Admin)
    exports["np-base"]:addModule("_Admin", dg._Admin)
end)