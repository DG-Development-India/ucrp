...
-- 					TriggerEvent('dg-society:getSociety', xPlayer.job.name, function (society)
...
-- 								else
-- 									TriggerClientEvent('dg:showAdvancedNotification', xPlayer.source, _U('bank'), '', _U('company_nomoney'), 'CHAR_BANK_MAZE', 1)
-- 								end
-- 							end)
-- 						else -- not a society
-- 							xPlayer.addAccountMoney('bank', salary)
...
-- DGCore.RegisterServerCallback('paycheck:checkSalary', function(source, cb)
-- 	if source ~= nil then
...
-- 						else
-- 							TriggerClientEvent('dg:showAdvancedNotification', source, _U('bank'), '', _U('company_nomoney'), 'CHAR_BANK_MAZE', 1)
-- 						end
-- 					end)
-- 				else -- not a society
-- 					xPlayer.addMoney( salary)
	...
	function payCheck()
		...
		for i=1, #xPlayers, 1 do
			...
			if salary > 0 then
				if job == 'unemployed' then -- unemployed
					xPlayer.addAccountMoney('bank', salary)
					TriggerClientEvent('dg:showAdvancedNotification', xPlayer.source, _U('bank'), _U('received_paycheck'), _U('received_help', salary), 'CHAR_BANK_MAZE', 9)
				else -- generic job
					xPlayer.addAccountMoney('bank', salary)
					TriggerClientEvent('dg:showAdvancedNotification', xPlayer.source, _U('bank'), _U('received_paycheck'), _U('received_salary', salary), 'CHAR_BANK_MAZE', 9)
				end
			end
		end
		SetTimeout(Config.PaycheckInterval, payCheck)
