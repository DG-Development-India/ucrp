TriggerEvent('es:addGroupCommand', 'setjob', 'admin', function(source, args, user)
	...
	if tonumber(args[1]) and args[2] and tonumber(args[3]) then
		...
		if xPlayer then
			if DGCore.DoesJobExist(args[2], args[3]) then
...
RegisterCommand('dg-setjob', function(source, args)
...
TriggerEvent('es:addGroupCommand', 'giveaccountmoney', 'admin', function(source, args, user)
	...
	if amount ~= nil then
		if xPlayer.getAccount(account) ~= nil then
			...
			TriggerClientEvent('dg:showNotification', _source, _U('invalid_account'))
		end
	else
		TriggerClientEvent('dg:showNotification', _source, _U('amount_invalid'))
	end
end, function(source, args, user)
	TriggerClientEvent('chatMessagess', source, 'SYSTEM: ', 3, 'Insufficient Permissions.')
...
RegisterCommand('dg-giveaccountmoney', function(source, args)
	if source == 0 or source =="console" then
		local xPlayer = DGCore.GetPlayerFromId(args[1])
		local account = args[2]
		local amount  = tonumber(args[3])
