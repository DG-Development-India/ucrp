Citizen.CreateThread(function()
	Citizen.Wait(500)
	print('\27[32m[dg-core]\27[0m Loaded All Data Successfully')
end)
	...
	TriggerEvent('es:getPlayerFromId', _source, function(player)
			...
			table.insert(tasks2, function(cb2)
				...
				MySQL.Async.fetchAll('SELECT job, job_grade, loadout, position FROM users WHERE identifier = @identifier', {
					...
					if DGCore.DoesJobExist(job, grade) then
						...
						print(('dg-core: %s had an unknown job [job: %s, grade: %s], setting as unemployed!'):format(player.getIdentifier(), job, grade))
		...
		Async.parallel(tasks, function(results)
			...
			xPlayer.getMissingAccounts(function(missingAccounts)
				...
				DGCore.Players[_source] = xPlayer

				TriggerEvent('dg:playerLoaded', _source, xPlayer)

				TriggerClientEvent('dg:playerLoaded', _source, {
...
AddEventHandler('playerDropped', function(reason)
	...
	if xPlayer then
		TriggerEvent('dg:playerDropped', _source, reason)
		TriggerEvent('dg:PlayerDropped', xPlayer, source)

		DGCore.SavePlayer(xPlayer, function()
			DGCore.Players[_source] = nil
			DGCore.LastPlayerData[_source] = nil
		end)
	end
end)

RegisterServerEvent('dg:updateLoadout')
AddEventHandler('dg:updateLoadout', function(loadout)
	local xPlayer = DGCore.GetPlayerFromId(source)
	xPlayer.loadout = loadout
end)

RegisterServerEvent('dg:updateLastPosition')
AddEventHandler('dg:updateLastPosition', function(position)
	local xPlayer = DGCore.GetPlayerFromId(source)
	xPlayer.setLastPosition(position)
end)

DGCore.RegisterServerCallback('dg:getPlayerData', function(source, cb)
	local xPlayer = DGCore.GetPlayerFromId(source)
...
DGCore.RegisterServerCallback('dg:getOtherPlayerData', function(source, cb, target)
