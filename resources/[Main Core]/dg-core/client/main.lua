...
Citizen.CreateThread(function()
	while true do
		...
		if paycheckReady == true and stackTwice == false and stackThree == false and stackfour == false and stackfive == false then
			DGCore.TriggerServerCallback('paycheck:checkSalary', function(result)
...
AddEventHandler('dg:playerLoaded', function(xPlayer)
	DGCore.PlayerLoaded = true
	DGCore.PlayerData = xPlayer

	if Config.EnableHud then
		TriggerEvent('es:setMoneyDisplay', 0.0)
		DGCore.UI.HUD.SetDisplay(0.0)
		for k,v in ipairs(xPlayer.accounts) do
			local accountTpl = '<div><img src="img/accounts/' .. v.name .. '.png"/>&nbsp;{{money}}</div>'

			DGCore.UI.HUD.RegisterElement('account_' .. v.name, k - 1, 0, accountTpl, {
				money = 0
			})

			DGCore.UI.HUD.UpdateElement('account_' .. v.name, {
				money = DGCore.Math.GroupDigits(v.money)
			})
		end
		...
		DGCore.UI.HUD.RegisterElement('job', #xPlayer.accounts, 0, jobTpl, {
			job_label   = '',
			grade_label = ''
		})

		DGCore.UI.HUD.UpdateElement('job', {
...
AddEventHandler('playerSpawned', function()
	while not DGCore.PlayerLoaded do
		Citizen.Wait(1)
	end
	...
	-- Restore position
	if DGCore.PlayerData.lastPosition then
		SetEntityCoords(playerPed, DGCore.PlayerData.lastPosition.x, DGCore.PlayerData.lastPosition.y, DGCore.PlayerData.lastPosition.z)
	end

	TriggerEvent('dg:restoreLoadout') -- restore loadout
...
AddEventHandler('dg:onPlayerDeath', function()
	isDead = true
end)
...
AddEventHandler('skinchanger:modelLoaded', function()
	...
	TriggerEvent('dg:restoreLoadout')
end)

RegisterNetEvent('dg:setAccountMoney')
AddEventHandler('dg:setAccountMoney', function(account)
	for k,v in ipairs(DGCore.PlayerData.accounts) do
		if v.name == account.name then
			DGCore.PlayerData.accounts[k] = account
			break
		end
	end

	if Config.EnableHud then
		DGCore.UI.HUD.UpdateElement('account_' .. account.name, {
			money = DGCore.Math.GroupDigits(account.money)
		})
	end
end)
...
-- Commands
RegisterNetEvent('dg:teleport')
AddEventHandler('dg:teleport', function(pos)
	pos.x = pos.x + 0.0
	pos.y = pos.y + 0.0
	pos.z = pos.z + 0.0
...
RegisterNetEvent('dg:loadIPL')
AddEventHandler('dg:loadIPL', function(name)
	Citizen.CreateThread(function()
		LoadMpDlcMaps()
		RequestIpl(name)
	end)
...
RegisterNetEvent('dg:unloadIPL')
AddEventHandler('dg:unloadIPL', function(name)
	Citizen.CreateThread(function()
		RemoveIpl(name)
	end)
end)

RegisterNetEvent('dg:playAnim')
AddEventHandler('dg:playAnim', function(dict, anim)
	Citizen.CreateThread(function()
		local playerPed = PlayerPedId()
...
RegisterNetEvent('dg:playEmote')
AddEventHandler('dg:playEmote', function(emote)
	Citizen.CreateThread(function()
...
RegisterNetEvent('dg:spawnVehicle')
AddEventHandler('dg:spawnVehicle', function(model)
	local playerPed = PlayerPedId()
	local coords    = GetEntityCoords(playerPed)

	DGCore.Game.SpawnVehicle(model, coords, 90.0, function(vehicle)
...
RegisterNetEvent('dg:spawnObject')
AddEventHandler('dg:spawnObject', function(model)
	local playerPed = PlayerPedId()
	local coords    = GetEntityCoords(playerPed)
	local forward   = GetEntityForwardVector(playerPed)
	local x, y, z   = table.unpack(coords + forward * 1.0)

	DGCore.Game.SpawnObject(model, {
		x = x,
		y = y,
		z = z
	}, function(obj)
...
RegisterNetEvent('dg:pickup')
AddEventHandler('dg:pickup', function(id, label, player)
	...
	--[[DGCore.Game.SpawnLocalObject('prop_money_bag_01', {
		x = x,
		y = y,
		z = z - 2.0,
	}, function(obj)
...
RegisterNetEvent('dg:removePickup')
AddEventHandler('dg:removePickup', function(id)
	DGCore.Game.DeleteObject(pickups[id].obj)
	pickups[id] = nil
end)

RegisterNetEvent('dg:spawnPed')
AddEventHandler('dg:spawnPed', function(model)
...
RegisterNetEvent('dg:deleteVehicle')
AddEventHandler('dg:deleteVehicle', function()
	local playerPed = PlayerPedId()
	...
	while true do
		...
		for k,v in pairs(pickups) do
			...
			if distance <= 1.0 and not v.inRange and IsPedOnFoot(playerPed) and IsControlJustReleased(0, Keys["E"]) then
				...
				TriggerServerEvent('dg:onPickup', v.id)
...
Citizen.CreateThread(function()
	while true do
		...
		if DGCore.PlayerLoaded and isPlayerSpawned then
			local coords = GetEntityCoords(playerPed)

			if not IsEntityDead(playerPed) then
				DGCore.PlayerData.lastPosition = {x = coords.x, y = coords.y, z = coords.z}
			end
		end
