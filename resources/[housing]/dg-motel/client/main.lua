    ...
    if motelDist < 200 then
     ...
     if entryDist < 1.2 then
      if rid.owner == PlayerId then
       ...
       --exports['dg-interior']:CreateApartmentFurnished(coords)
       showmenushit = true
       roomInfo = rid
       showClothing = true
      else
...
RegisterCommand('raidmotel', function(source, args)
 ...
 if DGCore.PlayerData.job and DGCore.PlayerData.job.name == 'police' then
  ...
  -- RegisterCommand("moteltime", function(source)
  --   TriggerServerEvent('ogrp_chairs:client:StartSit', source)
  -- end, false)
...
function OpenInv(owner, motel, room)
  print(motel)
  print(room)
  TriggerEvent("server-inventory-open", "1", "MT-"..motel.."-"..room)
...
Citizen.CreateThread(function()
  while true do
    ...
    for i=1, #Config.Complexs do
      ...
      if GetDistanceBetweenCoords(myCoords.x, myCoords.y, myCoords.z, complex.reception.x, complex.reception.y, complex.reception.z, false) < 20 then
        if GetDistanceBetweenCoords(myCoords.x, myCoords.y, myCoords.z, complex.reception.x, complex.reception.y, complex.reception.z, false) < 2.5 then
          if ownsMotel then
            if i == tonumber(myMotel.motel) then
              DrawText3D(complex.reception.x, complex.reception.y, complex.reception.z+0.25, "[E] - Renew your motel for $" .. complex.rent .. ". (" .. Config.Complexs[tonumber(myMotel.motel)].name .. " " .. myMotel.id .. ")")
            else
              DrawText3D(complex.reception.x, complex.reception.y, complex.reception.z+0.25, "You already own a motel")
            end
          else
          ...
          if IsControlJustPressed(0, 38) then
            ...
            if not ownsMotel then
              ...
              if totalRooms >= 1 then
                DGCore.TriggerServerCallback('motels:mycash', function(cash)
      ...
      if entryDist < 1.5 then
        if roomid.owner == PlayerId then
          if roomid.lock then
            ...
            isLocked = false
            if not pressed then
              DrawText3D(roomid.entry.x, roomid.entry.y, roomid.entry.z, "[E] - Enter | [G] - Lock | [M] - Sell")
              if IsControlJustReleased(0,  Keys['G']) then
              ...
              if IsControlJustReleased(0,  Keys['M']) then
                ...
                DGCore.UI.Menu.Open('default', GetCurrentResourceName(), 'Sell_Motel', {
                  title    = 'Sell Motel?',
                  align	= 'top-right',
                  elements = elements,
                }, function(data, menu)
                  ...
                  elseif data.current.value == 'yes' then
                    ...
                    DGCore.TriggerServerCallback('motels:GetOwnedMotel', function(result)
                      if result ~= nil then
                        print(result.room)
                        print(result.motelid)
                        TriggerServerEvent('pw-motels:cancelRoom', result.room, result.motelid)
                      end
                    end)
                    pressed = false
                  end
    ...
    if inmotel then
      ...
      if exitDist < 1.0 and isLocked then
        DrawText3D(myRoom.x - 1.15, myRoom.y - 4.2, myRoom.z + 1.20, "[E] - Leave | [G] - Unlock")
        if IsControlJustReleased(0, Keys['E']) then
        ...
        if IsControlJustReleased(0,  Keys['G']) then
          DGCore.TriggerServerCallback('motels:GetOwnedMotel', function(result)
            if result ~= nil then
              ...
              isLocked = false
              TriggerEvent('dooranim')
              TriggerServerEvent('pw-motels:toggleLock', result.motelid, result.room, true)
            end
          end)
        end
      end

      if exitDist < 1.0 and not isLocked then
        DrawText3D(myRoom.x - 1.15, myRoom.y - 4.2, myRoom.z + 1.20, "[E] - Leave | [G] - Lock")
        if IsControlJustReleased(0, Keys['E']) then
          TriggerServerEvent('hotel:deleteRoom', myRoom.id)
        end
        if IsControlJustReleased(0,  Keys['G']) then
      ...
      if showmenushit then
        ...
        --   DrawText3D(myRoom.x - 2, myRoom.y + 2.5, myRoom.z + 1.25, "[E] - Outfits")
        --   if IsControlJustReleased(0, Keys['E']) then
        --     TriggerEvent("ogrp_base:wardrobe")
        --   end
        -- end
              -- Stash
        ...
        if stashDist < 1.2 then
          DrawText3D(myRoom.x-1.6, myRoom.y+1.2, myRoom.z+1.25, "~g~[Main (Default: E)]~s~ to interact")
          if IsControlJustReleased(0, 38) then
            TriggerEvent('InteractSound_CL:PlayOnOne', 'zip', 0.8)
            Wait(500)
            OpenInv(roomInfo.ident, Config.Motel[roomInfo.motelid], roomInfo.roomno)
          end
        end
      end
    end
...
function buildHotel(generator)
 ...
 TriggerEvent('InteractSound_CL:PlayOnOne', 'doorenter', 0.8)
  ...
  builtRooms['building'] = CreateObject(GetHashKey("playerhouse_hotel"),generator.x-0.70,generator.y-0.40,generator.z-1.3, false, false, false)
  ...
  builtRooms['lamp'] = CreateObject(GetHashKey("apa_mp_h_lit_floorlamp_13"),generator.x-1.30,generator.y+3.90,generator.z+0.10, false, false, false)
  ...
  FreezeEntityPosition(builtRooms['frontDoor'], true)
	...
	SetEntityHeading(builtRooms['frontDoor'], GetEntityHeading(builtRooms['frontDoor'])+90)
...
RegisterNetEvent('hotel:check')
AddEventHandler('hotel:check', function()
  --local cid = exports['isPed']:isPed('cid')
  TriggerServerEvent('hotel:check')
end)
