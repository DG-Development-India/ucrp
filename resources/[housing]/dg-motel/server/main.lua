            ...
            if tonumber(xPlayer.getMoney()) >= tonumber(Config.Complexs[motel].price) then
                ...
                elseif motelowner[1] ~= nil then
                    ...
                    if Config.Complexs[motel].name == Config.Complexs[tonumber(motelowner[1].motelid)].name then
                        xPlayer.removeMoney(Config.Complexs[motel].rent)
                        TriggerClientEvent('DoLongHudText', src, 'Motel room '..room..' at '..Config.Complexs[motel].name..' has been renewed for $'..Config.Complexs[motel].rent)
                        MySQL.Sync.execute("UPDATE `pw_motels` SET days_left = days_left + 7 WHERE ident = @ident", {['@ident'] = ident})
                        Wait(1000)
                        TriggerEvent('hotel:check2', src)
                    else
...
AddEventHandler('pw-motels:cancelRoom', function(room, motel)
    ...
    MySQL.Sync.execute('DELETE FROM pw_motels WHERE ident = @ident', { ["@ident"] = ident})
    local motel_inv = 'MT-'..Config.Motel[motel].."-"..room..''
    MySQL.Sync.execute('DELETE FROM inventory_motel WHERE name = @motel_inv', {["@motel_inv"] = motel_inv})
    TriggerClientEvent('DoLongHudText', src, "You sold your motel room at "..Config.Complexs[motel].name)
    TriggerEvent('pw-motels:updateRooms')
end)
...
DGCore.RegisterServerCallback('pw-motels:myIdent', function(source, cb)
    local src = source
    local xPlayer = DGCore.GetPlayerFromId(src)
    local ident = xPlayer.identifier
    cb(ident)
...
DGCore.RegisterServerCallback('motels:mycash', function(source, cb)
    local src = source
    local xPlayer = DGCore.GetPlayerFromId(src)
        ...
        for i=1, #owners, 1 do
            ...
            for k,v in pairs(Config.Rooms) do
                if owners[i].room == tostring(v.roomno) and owners[i].motelid == v.motelid then
                    local xPlayer = DGCore.GetPlayerFromIdentifier(owner)
                    if xPlayer then
...
DGCore.RegisterServerCallback('motels:GetOwnedMotel', function(source, cb, cid)
    if cid ~= nil then
        MySQL.Async.fetchAll("SELECT * FROM pw_motels WHERE ident = @identifier ",{['@identifier'] = cid}, function(result)
        ...
        local src = source
        local Player = DGCore.GetPlayerFromId(src)
        MySQL.Async.fetchAll("SELECT * FROM pw_motels WHERE ident = @identifier", {['@identifier']= Player.identifier}, function(result)
            if result[1] ~= nil then 
                return cb(result[1])
            end
            return cb(nil)
...
DGCore.RegisterServerCallback('pw-motels:checkUserOnline', function(source, cb, motel, room)
    for k,v in pairs(Config.Rooms) do
        if motel == v.motelid and room == v.roomno then
            if v.ident == nil then
                cb(true)
            else
                local xPlayer = DGCore.GetPlayerFromIdentifier(v.ident)
                if xPlayer then
                    cb(true)
                else
                    cb(false)
...
RegisterServerEvent('hotel:check_client')
AddEventHandler('hotel:check_client', function(source)
    TriggerClientEvent('hotel:check', source)
end)

RegisterServerEvent('hotel:check')
AddEventHandler('hotel:check', function(source)
    ...
    MySQL.Async.fetchAll('SELECT * FROM pw_motels WHERE ident = @ident', {['@ident'] = ident}, function(motelowner)
        if motelowner[1] ~= nil then
            if motelowner[1].days_left <= 99 then
                TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'inform', text = 'Motel room '..motelowner[1].room..' at '..Config.Complexs[tonumber(motelowner[1].motelid)].name..' runs out in '..motelowner[1].days_left..' day(s). Renew it at the reception.', length = 15000})
            end
        end
    end)
    MySQL.Async.fetchAll('SELECT * FROM apartments WHERE identifier = @ident', {['@ident'] = ident}, function(aptowner)
        if aptowner[1] ~= nil then
            if aptowner[1].days_left <= 99 then
                TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'inform', text = 'Your apartment at '..aptowner[1].label..' runs out in '..aptowner[1].days_left..' day(s). Renew it at the entrance.', length = 15000})
            end
        end
    end)
end)

RegisterServerEvent('hotel:check2')
AddEventHandler('hotel:check2', function(source)
    Wait(1000)
    local src = source
    local xPlayer = DGCore.GetPlayerFromId(src)
    local ident = xPlayer.identifier
    MySQL.Async.fetchAll('SELECT * FROM pw_motels WHERE ident = @ident', {['@ident'] = ident}, function(motelowner)
        if motelowner[1] ~= nil then
            TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'inform', text = 'Motel rent of Room '..motelowner[1].room..' at '..Config.Complexs[tonumber(motelowner[1].motelid)].name..' has been paid for '..motelowner[1].days_left..' day(s).', length = 15000})
        end
    end)
end)
 ...
 MySQL.Async.fetchAll('SELECT * FROM pw_motels', {}, function(res)
  for id,v in pairs(res) do
   if v.days_left > 0 then
    ...
    local motel_inv = 'MT-'..Config.Motel[v.motel].."-"..v.room..''
    MySQL.Sync.execute('DELETE FROM inventory_motel WHERE name = @motel_inv', {["@motel_inv"] = motel_inv})
   end
  end
  TriggerEvent('pw-motels:updateRooms')
...
TriggerEvent('cron:runAt', 06, 00, CronTask)
